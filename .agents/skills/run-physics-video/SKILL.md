...
- If `ue_launch_budget_exhausted` is returned, report the used and current maximum UE launches and obtain explicit user approval for a higher absolute limit. Then call `resume <job_id> --max-ue-launches <new-limit>`; never reset usage or choose the increase without approval.
- If `case_spec_revision_budget_exhausted` is returned, report the used and current maximum revisions and obtain explicit user approval for a higher absolute limit. Call `resume <job_id> --max-case-spec-revisions <new-limit>`, then submit the authorized revised CaseSpec and reason through the returned `resume_with_revision` action; the two may also be supplied in one resume call. Never reset revision history or increase the limit without approval.
...
- For an ordinary CaseSpec-invalid compile blocker, inspect `case_spec_revision_policy.allowed_adjustments`. It is the Controller-derived bounded policy for Agent-inferred layout leaves (object position/rotation and static support size) plus the Intent-declared soft parameters. Revise only the smallest relevant listed leaves; never enlarge a support or move objects merely to hide a compiler defect or weaken the requested layout.
