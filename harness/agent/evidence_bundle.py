...
def build_evidence_bundle(
    *,
    ...
    if manifest_path.is_file():
        ...
        prune_validated_sensor_exr(run_dir)
        return {
            "manifest": manifest,
    ...
    selection = _select_event_points(frames, timeline, snapshots["case_spec"])
    views = _canonical_views(run_dir, snapshots["case_spec"])
    if not views:
    ...
    deformable_state_truth = _deformable_state_truth(attempt_dir, run_dir, trajectory_summary)
    explicit_asset_bindings = _explicit_asset_bindings(
        attempt_dir,
        run_dir,
        snapshots["case_spec"],
    )
    ...
    write_json(
        summary_path,
        {
            ...
            "deformable_state_truth": deformable_state_truth,
            "constraint_states": _constraint_state_summary(frames, trajectory_summary),
            "contacts": timeline,
            "technical_gates": technical_gates,
            "result_provenance": dict(result_provenance),
            "explicit_asset_bindings": explicit_asset_bindings,
        },
    )
...
def prune_validated_sensor_exr(run_dir: str | Path) -> dict[str, Any]:
    run_dir = Path(run_dir).resolve(strict=True)
    quality = read_json(run_dir / "quality_report.json")
    if quality.get("hard_gate_passed") is not True:
        raise EvidenceBundleError("sensor_retention_before_quality", "Sensor EXR retention requires a passed quality gate")

    removed: list[dict[str, Any]] = []
    for view_dir in sorted((run_dir / "views").glob("*")):
        if not view_dir.is_dir() or view_dir.is_symlink():
            continue
        for name in ("depth_frames", "segmentation_frames"):
            path = view_dir / name
        ...
        for name in ("depth.exr", "segmentation.exr"):
            path = view_dir / name
            if not path.is_file() or path.is_symlink():
                continue
            size_bytes = path.stat().st_size
            path.unlink()
            removed.append({"path": path.relative_to(run_dir).as_posix(), "file_count": 1, "size_bytes": size_bytes})
        meta_path = view_dir / "meta.json"
        if meta_path.is_file():
            meta = read_json(meta_path)
            meta["depth_frames"] = []
            meta["segmentation_frames"] = []
            meta["validated_sensor_exr_retained"] = False
            write_json(meta_path, meta)

    report = {
        "schema_version": "harness_sensor_artifact_retention_v1",
        "status": "pass",
        "policy": "retain_rgb_reports_metrics_and_previews_prune_validated_exr",
        "removed": removed,
        "removed_size_bytes": sum(int(row["size_bytes"]) for row in removed),
        "updated_at": utc_now(),
    }
    write_json(run_dir / "sensor_artifact_retention.json", report)
    return report
...
def _select_event_points(
    frames: list[dict[str, Any]],
    timeline: list[dict[str, Any]],
    case_spec: Mapping[str, Any],
) -> dict[str, Any]:
    verification = case_spec.get("verification_requirements")
    assertions = verification.get("assertions") if isinstance(verification, Mapping) else []
    for assertion in assertions or []:
        if not isinstance(assertion, Mapping) or assertion.get("type") != "event_sequence":
            continue
        raw_pairs = assertion.get("pairs")
        if isinstance(raw_pairs, list):
            pairs = [pair for pair in raw_pairs if isinstance(pair, list) and len(pair) == 2]
        else:
            objects = assertion.get("objects")
            pairs = list(zip(objects, objects[1:], strict=False)) if isinstance(objects, list) else []
        matching_events = [
            [event for event in timeline if set(event.get("objects") or []) == set(pair)]
            for pair in pairs
        ]
        if len(matching_events) == 2 and all(matching_events):
            second_event = matching_events[1][0]
            first_episode = [
                event
                for event in matching_events[0]
                if int(event["frame_index"]) < int(second_event["frame_index"])
            ]
            if first_episode:
                support_end = first_episode[-1]
                support_start_index = int(first_episode[0]["frame_index"])
                support_end_index = int(support_end["frame_index"])
                new_contact_index = int(second_event["frame_index"])
                midpoint_index = (support_end_index + new_contact_index) // 2
                if support_start_index < support_end_index:
                    supported_motion_index = (support_start_index + support_end_index) // 2
                    supported_motion_event = min(
                        first_episode,
                        key=lambda event: abs(int(event["frame_index"]) - supported_motion_index),
                    )
                    return _event_selection(
                        frames,
                        [supported_motion_index, support_end_index, midpoint_index, new_contact_index],
                        strategy="event_sequence_transition",
                        reason=f"anchored to verification assertion {assertion.get('id') or 'event_sequence'}",
                        labels=("supported_motion", "support_end", "unsupported_midpoint", "new_contact"),
                        event_refs=(
                            str(supported_motion_event["event_id"]),
                            str(support_end["event_id"]),
                            "",
                ...
                return _event_selection(
                    frames,
                    [support_end_index, midpoint_index, new_contact_index],
                    strategy="event_sequence_transition",
                    reason=f"anchored to verification assertion {assertion.get('id') or 'event_sequence'}",
                    labels=("support_end", "unsupported_midpoint", "new_contact"),
                    event_refs=(str(support_end["event_id"]), "", str(second_event["event_id"])),
                )
        matched = [events[0] if events else None for events in matching_events]
        if not matched or any(event is None for event in matched):
            continue
        sequence = [event for event in matched if event is not None]
        indices = [int(event["frame_index"]) for event in sequence]
        if any(before >= after for before, after in zip(indices, indices[1:], strict=False)):
            continue
        middle_event = sequence[(len(sequence) - 1) // 2]
        point_indices = [
            max(0, indices[0] - 1),
            int(middle_event["frame_index"]),
            min(len(frames) - 1, indices[-1] + 1),
        ]
        return _event_selection(
            frames,
            point_indices,
            ...
            reason=f"anchored to verification assertion {assertion.get('id') or 'event_sequence'}",
            event_ref=str(middle_event["event_id"]),
        )

    point_count = min(len(frames), _MAX_EVIDENCE_IMAGES // _MAX_EVIDENCE_VIEWS)
    indices = sorted(
        {
            round(slot * (len(frames) - 1) / max(1, point_count - 1))
            for slot in range(point_count)
        }
    )
    return _event_selection(
        frames,
        indices,
        strategy="uniform_interval",
        reason="no event_sequence transition was selected; deterministic uniform sampling",
        labels=tuple(f"sample_{index:02d}" for index in range(len(indices))),
        event_refs=tuple("" for _ in indices),
    )
...
def _event_selection(
    ...
    *,
    ...
    point_labels = tuple(labels or ("before", "during", "after"))
    point_event_refs = tuple(event_refs or ("", event_ref, ""))
    return {
        "strategy": strategy,
        "reason": reason,
        "points": [
            {
                ...
                "frame_index": index,
                "event_refs": [point_event_refs[offset]] if point_event_refs[offset] else [],
            }
            for offset, (label, index) in enumerate(zip(point_labels, indices))
        ],
    }


def _constraint_state_summary(
    frames: list[dict[str, Any]],
    trajectory_summary: Mapping[str, Any],
) -> dict[str, Any]:
    sampled_indices = {
        int(sample["frame_index"])
        for sample in trajectory_summary.get("sampled_frames") or []
        if isinstance(sample, Mapping) and isinstance(sample.get("frame_index"), int)
    }
    constraint_ids: set[str] = set()
    samples: list[dict[str, Any]] = []
    for frame_index in sorted(sampled_indices):
        constraints = frames[frame_index].get("constraints")
        rows = (
            [dict(value) for value in constraints if isinstance(value, Mapping)]
            if isinstance(constraints, list)
            else []
        )
        if not rows:
            continue
        constraint_ids.update(str(value.get("constraint_id")) for value in rows if value.get("constraint_id"))
        samples.append(
            {
                "frame_index": frame_index,
                "time_s": float(_frame_time(frames[frame_index]) or 0.0),
                "constraints": rows,
            }
        )
    return {
...
def _canonical_views(run_dir: Path, case_spec: Mapping[str, Any]) -> list[str]:
    quality = read_json(run_dir / "quality_report.json")
    ...
    available = [view for view in candidates if (run_dir / "views" / view / "rgb.mp4").is_file()]
    observation = case_spec.get("observation_requirements")
    cameras = observation.get("cameras") if isinstance(observation, Mapping) else []
    declared = {
        str(camera.get("role") or camera.get("camera_id") or "")
        for camera in cameras or []
        if isinstance(camera, Mapping)
    }
    if declared:
        available = [view for view in available if view in declared]

    def information_score(view_id: str) -> tuple[int, str]:
        row = media_views.get(view_id) if isinstance(media_views, Mapping) else {}
        rgb = row.get("rgb") if isinstance(row, Mapping) else {}
        return (-int((rgb or {}).get("bitrate_bps") or 0), view_id)

    return sorted(available, key=information_score)[:_MAX_EVIDENCE_VIEWS]
...
def _deformable_state_truth(
    attempt_dir: Path,
    run_dir: Path,
    trajectory_summary: Mapping[str, Any],
) -> dict[str, Any] | None:
    path = run_dir / "particle_cache.json"
    if not path.is_file():
        return None
    payload = read_json(path)
    raw_frames = payload.get("frames") if isinstance(payload, Mapping) else None
    frames = [row for row in raw_frames or [] if isinstance(row, Mapping)] if isinstance(raw_frames, list) else []
    if not frames:
        return None

    sampled: list[dict[str, Any]] = []
    for trajectory_sample in trajectory_summary.get("sampled_frames") or []:
        if not isinstance(trajectory_sample, Mapping):
            continue
        trajectory_time = _number_or_none(trajectory_sample.get("time_s"))
        if trajectory_time is None:
            continue
        frame = min(
            frames,
            key=lambda row: abs(float(_number_or_none(row.get("time_s")) or 0.0) - trajectory_time),
        )
        positions = _finite_vectors(frame.get("positions_m"), width=3)
        if not positions:
            continue
        velocities = _finite_vectors(frame.get("velocities_m_s"), width=3)
        speeds = [math.sqrt(sum(component * component for component in value)) for value in velocities]
        sampled.append(
            {
                "trajectory_frame_index": int(trajectory_sample["frame_index"]),
                "particle_frame_index": _integer(frame.get("frame"), default=len(sampled)),
                "time_s": float(trajectory_time),
                "particle_time_s": float(_number_or_none(frame.get("time_s")) or 0.0),
                "particle_count": len(positions),
                "centroid_m": [sum(value[axis] for value in positions) / len(positions) for axis in range(3)],
                "bounds_m": {
                ...
                "measurements": dict(frame.get("measurements") or {})
                if isinstance(frame.get("measurements"), Mapping)
                else {},
            }
        )
    if not sampled:
        return None
    object_ids = set(str(value) for value in trajectory_summary.get("objects") or [])
    return {
        "schema_version": "harness_deformable_state_summary_v1",
        "authority": "particle_cache",
        "subject_id": "fluid_surface" if "fluid_surface" in object_ids else "deformable_surface",
        "source_path": path.resolve().relative_to(attempt_dir).as_posix(),
        "source_sha256": _sha256_file(path),
        "frame_count": len(frames),
...
def _finite_vectors(value: Any, *, width: int) -> list[list[float]]:
    ...
    for row in value:
        if not isinstance(row, (list, tuple)) or len(row) != width:
            continue
        vector = [_number_or_none(component) for component in row]
        if all(component is not None for component in vector):
            result.append([float(component) for component in vector])
    return result


def _explicit_asset_bindings(
    attempt_dir: Path,
    run_dir: Path,
    case_spec: Mapping[str, Any],
) -> dict[str, Any]:
    resolution_path = run_dir / "asset_resolution.json"
    input_manifest_path = run_dir / "provider_input_manifest.json"
    resolution = read_json(resolution_path) if resolution_path.is_file() else {}
    input_manifest = read_json(input_manifest_path) if input_manifest_path.is_file() else {}
    resolution_rows = resolution.get("assets") if isinstance(resolution, Mapping) else []
    input_rows = input_manifest.get("inputs") if isinstance(input_manifest, Mapping) else []
    ...
    for object_row in case_spec.get("objects") or []:
        if not isinstance(object_row, Mapping):
            continue
        asset = object_row.get("asset")
        acquisition = asset.get("acquisition") if isinstance(asset, Mapping) else None
        if not isinstance(acquisition, Mapping) or acquisition.get("requirement") != "required" or acquisition.get("origin") != "user_explicit":
            continue
        object_id = str(object_row.get("id") or "")
        resolution_row = next(
            (
                row
                ...
                and str(
                    row.get("object_id")
                    or ((row.get("intent") or {}).get("object_id") if isinstance(row.get("intent"), Mapping) else "")
                )
                == object_id
            ),
            {},
        )
        selected = resolution_row.get("selected_asset") if isinstance(resolution_row, Mapping) else None
        selected = selected if isinstance(selected, Mapping) else {}
        source_uri = str(selected.get("source_uri") or acquisition.get("source_uri_hint") or "")
        ...
        input_row = next(
            (
                ...
                if isinstance(row, Mapping)
                and (
                    (source_uri and str(row.get("source_uri") or "") == source_uri)
                    or (sha256 and str(row.get("sha256") or "") == sha256)
                )
            ),
            {},
        )
        geometry_registration = selected.get("geometry_registration")
        geometry_registration = geometry_registration if isinstance(geometry_registration, Mapping) else {}
        ...
        bindings.append(
            {
                ...
                "sha256": sha256 or input_row.get("sha256"),
                "selection_reason": resolution_row.get("selection_reason")
                if isinstance(resolution_row, Mapping)
                else None,
                "input_id": input_row.get("input_id") if isinstance(input_row, Mapping) else None,
                "input_local_path": input_row.get("local_path") if isinstance(input_row, Mapping) else None,
                "ue_path": selected.get("ue_path") or ue.get("object_path"),
                "geometry_registration_status": geometry_registration.get("status"),
            }
        )
    sources = []
    for path in (resolution_path, input_manifest_path):
        if path.is_file():
            sources.append(
                {
                    "path": path.resolve().relative_to(attempt_dir).as_posix(),
    ...
    return {
        "schema_version": "harness_explicit_asset_bindings_v1",
        "sources": sources,
        "items": bindings,
    }
