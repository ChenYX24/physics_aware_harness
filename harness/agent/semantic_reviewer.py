...
class CodexAppServerReviewer:
    ...
    def _input_items(bundle_dir: Path, manifest: Mapping[str, Any], *, include_original_images: bool) -> list[dict[str, Any]]:
        prompt = (
            ...
            "Treat evidence_summary.json result_provenance as hashed result evidence, not as requested intent. "
            ...
            "Set overall_status to uncertain if any requirement is uncertain, otherwise fail if any requirement fails, "
            "otherwise pass. Use repair_layer=none only for an overall pass; every fail or uncertain result requires a "
            "non-none repair layer. "
            "Every evidence_refs item must set at least one concrete locator: time_s, view_id, trajectory_range, or "
            "contact_event_id; the four locator fields must not all be null. Copy locator values exactly from manifest.json. "
            "A keyframe citation requires both its exact time_s and view_id. A multi_view_montage citation requires its exact "
            "time_s. A structured_summary citation may use an exact sampled/event time_s, an exact readable trajectory_range, "
            "or an existing contact_event_id. Input snapshots cannot support a pass verdict by themselves. "
            "Suggested adjustments are optional. If you include one, copy path exactly from "
            "inputs/intent_contract.json allowed_adjustments.paths; otherwise return an empty list. Never invent a path, "
            "use an array index, repeat a path, or suggest a broad parent object. suggested_adjustments evidence_refs contain artifact_id "
            "strings only. "
            "For trajectory evidence, inspect trajectory_summary.sampled_frames, readable_ranges, and state_transitions; "
            "for deforming surfaces, treat evidence_summary.json deformable_state_truth as authoritative over a fixed replay "
            "Actor transform, and use explicit_asset_bindings for required user-explicit asset identity. "
