...
ADVANCE_STAGES = tuple(_NEXT_CONTROLLER_STAGE)
...
class AgentJobController:
    ...
    def create(
        ...
        *,
        ...
        if not isinstance(ignore_provenance_and_release_gates, bool):
            raise ValueError("ignore_provenance_and_release_gates must be boolean")
        provider_manifest = (
    ...
    def advance_until_blocked(
        ...
        *,
        ...
        if stop_after_stage is not None and stop_after_stage not in ADVANCE_STAGES:
            raise ValueError(f"unsupported stop-after stage: {stop_after_stage}")
        with self.store.lock(job_id):
            manifest = self.store.load_manifest(job_id)
            ...
            while manifest["state"] == "running":
                ...
                if (
                    ...
                    and manifest["state"] == "running"
                    and stop_after_stage == stage
                ):
                    manifest = self._update_manifest(
                        manifest,
                        state="blocked",
                        blocker={
                            "code": "requested_stage_boundary_reached",
                            "message": f"requested stop after completed stage {stage}",
                            "stage": stage,
                        },
                        allowed_next_actions=["advance", "cancel"],
                    )
                    self._stage_event(manifest, "stage_boundary_reached", stage)
                    break
    ...
    def resume(
        ...
        *,
        ...
        max_case_spec_revisions: int | None = None,
        authorizations: Mapping[str, Any] | None = None,
        ...
        with self.store.lock(job_id):
            ...
            if (
                ...
                and max_case_spec_revisions is None
            ...
            revision_budget_blocked = (
                (manifest.get("blocker") or {}).get("code") == "case_spec_revision_budget_exhausted"
            )
            revision_failure_class = str(
                (((leaf or {}).get("result") or {}).get("failure_class") or "")
            )
            revision_budget_extension = (
                max_case_spec_revisions is not None
                and manifest["usage"]["case_spec_revisions"] >= manifest["budget"]["max_case_spec_revisions"]
                and (
                    revision_budget_blocked
                    or (manifest.get("blocker") or {}).get("code")
                    in {"semantic_intent_mismatch", "semantic_review_uncertain"}
                    or revision_failure_class
                    in {"case_spec_invalid", "verification_failed", "render_sync_failed", "quality_gate_failed"}
                )
            )
            if max_case_spec_revisions is not None and revised_case_spec is None and not revision_budget_extension:
                raise JobStoreError("max_case_spec_revisions requires revised_case_spec")
            requested_action = "resume_with_revision" if revised_case_spec is not None else "resume"
            if (
                requested_action not in manifest["allowed_next_actions"]
                and not contract_revision
                and not revision_budget_extension
            ):
            ...
            if (manifest.get("blocker") or {}).get("code") == "ue_launch_budget_exhausted" and max_ue_launches is None:
                raise JobStoreError("resuming an exhausted UE launch budget requires max_ue_launches")
            if (manifest.get("blocker") or {}).get("code") == "case_spec_revision_budget_exhausted":
                if max_case_spec_revisions is None:
                    raise JobStoreError(
                        "resuming an exhausted CaseSpec revision budget requires max_case_spec_revisions"
                    )
            ...
            if max_paid_submissions is not None:
                ...
                manifest["budget"] = normalized_budget(budget)
            if max_ue_launches is not None:
                if (
                    not isinstance(max_ue_launches, int)
                    or isinstance(max_ue_launches, bool)
                    or max_ue_launches <= max(
                        manifest["budget"]["max_ue_launches"],
                        int(manifest["usage"]["ue_launches"]),
                    )
                ):
                    raise ValueError("max_ue_launches must exceed the current limit and used launch count")
                budget = dict(manifest["budget"])
                budget["max_ue_launches"] = max_ue_launches
                manifest["budget"] = normalized_budget(budget)
            if max_case_spec_revisions is not None:
                if (
                    not isinstance(max_case_spec_revisions, int)
                    or isinstance(max_case_spec_revisions, bool)
                    or max_case_spec_revisions <= max(
                        manifest["budget"]["max_case_spec_revisions"],
                        int(manifest["usage"]["case_spec_revisions"]),
                    )
                ):
                    raise ValueError(
                        "max_case_spec_revisions must exceed the current limit and used revision count"
                    )
                budget = dict(manifest["budget"])
                budget["max_case_spec_revisions"] = max_case_spec_revisions
                manifest["budget"] = normalized_budget(budget)
            if authorizations:
            ...
            if (
                ...
                or manifest["budget"]["max_ue_launches"] != prior_budget["max_ue_launches"]
                or manifest["budget"]["max_case_spec_revisions"] != prior_budget["max_case_spec_revisions"]
            ):
            ...
            if revision_budget_extension and revised_case_spec is None:
                self._update_manifest(
                    manifest,
                    state="needs_user_decision",
                    current_stage=("budget" if revision_budget_blocked else manifest["current_stage"]),
                    blocker=(
                        {
                            "code": "case_spec_revision_required",
                            "message": "CaseSpec revision budget was extended; submit the revised CaseSpec",
                            "stage": "budget",
                        }
                        if revision_budget_blocked
                        else manifest["blocker"]
                    ),
        ...
        if resume_to_review_boundary:
            return self.inspect(job_id)
        return self.advance_until_blocked(job_id, stop_after_stage=stop_after_stage)
    ...
    def _advance_l1(self, manifest: dict[str, Any]) -> dict[str, Any]:
        ...
        ignore_release_gates = self._ignore_provenance_and_release_gates(manifest)
        if blocker is None and not ignore_release_gates and target_tier == "reference" and case_tier != "reference":
            blocker = ("publication_tier_not_satisfied", "CaseSpec must require reference-tier assets")
        if blocker is None and not ignore_release_gates and target_tier == "reference" and selection["selected_backend"] == "fallback":
    ...
    def _advance_run(
        ...
        *,
        ...
        write_json(
            expected_run / "quality_policy.json",
            {
                "schema_version": "harness_quality_policy_v1",
                "ignore_provenance_and_release_gates": self._ignore_provenance_and_release_gates(manifest),
            },
        )
        handoff_contract = compilation.backend_selection.get("handoff_contract")
        particle_handoff_root = (
            attempt_dir / "physics_handoff"
            if isinstance(handoff_contract, Mapping)
            and handoff_contract.get("contract_id") == "particle_surface_cache_v1"
            else None
        )
        started = self.hooks.monotonic()
                ...
                with self._profile_environment(profile.environment() if ue_required else {}):
                    run_dir = Path(
                        self.hooks.execute(
                            ...
                            complete_sensor_contract={"rgb", "depth", "segmentation"}.issubset(planned_passes),
                            physics_handoff_root=particle_handoff_root,
                        )
                    )
            except BaseException as exc:
            ...
            render_sync = (
                ...
                and read_json(render_stage_path).get("status") in {"completed", "failed", "blocked"}
                    ...
                    self.hooks.render_sync(
                        run_dir,
                        require_depth="depth" in planned_passes,
                        require_segmentation="segmentation" in planned_passes,
                        write=True,
                    )
                )
            )
    ...
    def _evidence_result_provenance(
        ...
        *,
        ...
        if achieved_tier not in {"diagnostic_only", "local_preview", "reference"}:
            if ignore_release_gates:
                achieved_tier = "local_preview"
            else:
            ...
            assets.append(
                {
                    ...
                    "complete": bool(selected and provenance),
                }
            )
        routes = sorted(
            {
        ...
        return {
            ...
            "publication": {
                ...
                "provenance_and_release_gate_override": ignore_release_gates,
                "quality_report": {
    ...
    def _record_authorization_amendment(
        ...
        *,
        ...
        if prior_budget["max_ue_launches"] != manifest["budget"]["max_ue_launches"]:
            budget_changes["max_ue_launches"] = {
                "before": prior_budget["max_ue_launches"],
                "after": manifest["budget"]["max_ue_launches"],
            }
        if prior_budget["max_case_spec_revisions"] != manifest["budget"]["max_case_spec_revisions"]:
            budget_changes["max_case_spec_revisions"] = {
                "before": prior_budget["max_case_spec_revisions"],
                "after": manifest["budget"]["max_case_spec_revisions"],
            }
        request = read_json(root / "user_request.json")
    ...
    def _case_spec_revision_budget_result(
        ...
        *,
        ...
        return build_stage_result(
            ...
            message="CaseSpec revision budget is exhausted; approve a higher limit before another revision",
            retryable=False,
            job_id=str(manifest["job_id"]),
            attempt_id=manifest.get("current_attempt_id"),
            artifact_refs=refs,
            allowed_next_actions=["revise_case_spec", "inspect_artifacts", "cancel"],
            required_user_action={
                "code": "case_spec_revision_budget_exhausted",
                "message": "approve a higher max_case_spec_revisions and submit the revised CaseSpec",
            },
        )
    ...
    def _block_case_spec_revision_budget(
        ...
        *,
        ...
        updated = self._update_manifest(
            ...
            allowed_next_actions=["resume", "resume_with_revision", "inspect_artifacts", "cancel"],
        )
    ...
    def _apply_stage_result(self, manifest: dict[str, Any], result: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if value["failure_class"] in {"blocked_user_action", "blocked_configuration"}:
            ...
            if value["failure_code"] in {
                ...
                actions = ["resume", "resume_with_revision", "inspect_artifacts", "cancel"]
            elif value["failure_code"] == "ue_launch_budget_exhausted":
                actions = ["resume", "inspect_artifacts", "cancel"]
            return self._update_manifest(
                manifest,
                state=state,
                blocker={"code": value["failure_code"], "message": value["message"], "stage": value["stage"]},
    ...
    def _effective_environment(self, manifest: Mapping[str, Any] | None = None):
        ...
        if manifest is not None and self._ignore_provenance_and_release_gates(manifest):
            values["SIM_HARNESS_IGNORE_PROVENANCE_AND_RELEASE_GATES"] = "1"
    ...
    def _case_spec_revision_policy(
        ...
        *,
        ...
        def add_vector_bounds(path: str, value: Any, *, minimum: float, maximum: float) -> None:
            if path in excluded or not cls._finite_numeric_vector(value, length=3):
                return
            constraints[path] = {
                "kind": "numeric_vector",
        ...
        def add_enum(path: str, value: Any, *, values: list[str]) -> None:
            if path in excluded or value not in values:
                return
            constraints[path] = {"kind": "enum", "values": values}

        add_numeric_window(
            "$.scene.duration_s",
            (case_spec.get("scene") or {}).get("duration_s"),
            relative=0.5,
            floor=0.5,
        )
        cameras = (case_spec.get("observation_requirements") or {}).get("cameras")
        if "$.observation_requirements.cameras" not in excluded and isinstance(cameras, list) and cameras:
            constraints["$.observation_requirements.cameras"] = {
                "kind": "list",
                "min_items": len(cameras),
                "max_items": len(cameras),
            }
        ...
        for obj in case_spec.get("objects") or []:
            ...
            if body_type == "dynamic":
                ...
                add_numeric_window(
                    f"$.objects.{object_id}.physics.material.density_kg_m3",
            ...
            if solver.get("type") == "articulated_body":
                pose_source = solver.get("pose_source") if isinstance(solver.get("pose_source"), Mapping) else {}
                pose_overlay = solver.get("pose_overlay") if isinstance(solver.get("pose_overlay"), Mapping) else {}
                root_source = solver.get("root_transform_source") if isinstance(solver.get("root_transform_source"), Mapping) else {}
                head_look = solver.get("head_look_target") if isinstance(solver.get("head_look_target"), Mapping) else {}
                articulated_lists = (
                    ("pose_source.keyframes", pose_source.get("keyframes")),
                    ("pose_source.segments", pose_source.get("segments")),
                    ("pose_overlay.keyframes", pose_overlay.get("keyframes")),
                    ("root_transform_source.keyframes", root_source.get("keyframes")),
                    ("ik_targets", solver.get("ik_targets")),
                    ("head_look_target.keyframes", head_look.get("keyframes")),
                    ("attachments", solver.get("attachments")),
                )
                for field, value in articulated_lists:
                    path = f"$.objects.{object_id}.solver.{field}"
                    if path not in excluded and isinstance(value, list):
                        constraints[path] = {
                            "kind": "list",
                            "min_items": len(value),
                            "max_items": len(value),
                        }
            motion = solver.get("motion") if isinstance(solver.get("motion"), Mapping) else {}
            motion_path = f"$.objects.{object_id}.solver.motion"
            add_numeric_window(
                f"{motion_path}.start_time_s",
                motion.get("start_time_s"),
                relative=0.5,
                floor=0.25,
            )
            add_numeric_window(
                f"{motion_path}.duration_s",
                motion.get("duration_s"),
                relative=0.5,
                floor=0.25,
            )
            add_vector_window(
                f"{motion_path}.pivot_local_m",
                motion.get("pivot_local_m"),
                relative=0.5,
                floor=0.05,
            )
            for field in ("solver_end_rotation_xyz_deg", "ue_end_rotation_pyr_deg"):
                add_vector_bounds(
                    f"{motion_path}.{field}",
                    motion.get(field),
                    minimum=-180.0,
        ...
        for constraint in case_spec.get("constraints") or []:
            ...
            if not _CANONICAL_OBJECT_PATH_SEGMENT.fullmatch(constraint_id):
                continue
            constraint_path = f"$.constraints.{constraint_id}"
            for frame_name in ("frame_a", "frame_b"):
                frame = constraint.get(frame_name) if isinstance(constraint.get(frame_name), Mapping) else {}
                frame_path = f"{constraint_path}.{frame_name}"
                add_vector_window(
                    f"{frame_path}.position_m",
                    frame.get("position_m"),
                    relative=0.5,
                    floor=0.05,
                )
                for axis_name in ("primary_axis", "secondary_axis"):
                    add_vector_bounds(
                        f"{frame_path}.{axis_name}",
                        frame.get(axis_name),
                        minimum=-1.0,
                        maximum=1.0,
                    )
            add_numeric_window(
                f"{constraint_path}.linear_limit_m",
                constraint.get("linear_limit_m"),
                relative=0.5,
            )
            distance_spring = (
                constraint.get("unilateral_distance_spring")
                if isinstance(constraint.get("unilateral_distance_spring"), Mapping)
                else {}
            )
            add_numeric_window(
                f"{constraint_path}.unilateral_distance_spring.rest_length_m",
                distance_spring.get("rest_length_m"),
                relative=0.5,
            )
            add_numeric_window(
                f"{constraint_path}.unilateral_distance_spring.stiffness_n_m",
                distance_spring.get("stiffness_n_m"),
                relative=0.75,
            )
            add_numeric_window(
                f"{constraint_path}.unilateral_distance_spring.damping_n_s_m",
                distance_spring.get("damping_n_s_m"),
                relative=0.75,
                floor=0.5,
            )
            angular_limits = (
                constraint.get("angular_limits_deg")
                if isinstance(constraint.get("angular_limits_deg"), Mapping)
                else {}
            )
            for axis_name in ("x", "y", "z"):
                add_numeric_window(
                    f"{constraint_path}.angular_limits_deg.{axis_name}",
                    angular_limits.get(axis_name),
                    relative=0.5,
                    floor=5.0,
                )
            axial_visual = (
                constraint.get("axial_visual")
                if isinstance(constraint.get("axial_visual"), Mapping)
                else {}
            )
            add_enum(
                f"{constraint_path}.axial_visual.mesh_forward_axis",
                axial_visual.get("mesh_forward_axis"),
                values=["x", "y", "z"],
            )
    ...
    def _publication_tier_satisfied(self, manifest: Mapping[str, Any]) -> bool:
        if self._ignore_provenance_and_release_gates(manifest):
            return True
    ...
    @staticmethod
    def _ignore_provenance_and_release_gates(manifest: Mapping[str, Any]) -> bool:
        target = manifest.get("target") if isinstance(manifest.get("target"), Mapping) else {}
        return target.get("ignore_provenance_and_release_gates") is True
