...
class CaseGenerationV2Tests(unittest.TestCase):
    ...
    def test_exactly_two_normal_calls_generate_v2(self) -> None:
        ...
        self.assertEqual(
            ...
            {"articulated_body_motion", "rigid_body_dynamics", "fluid_particle_dynamics", "deformable_body_dynamics"},
        )
        self.assertEqual(
        ...
        self.assertIn("activation_distance_m", contract["field_shapes"]["compliant_contact"])
        ...
        self.assertIn("Once declared, collision_geometry is the\n   collision truth", case_prompt)
        ...
        self.assertIn("corresponding primary/secondary axes must match", case_prompt)
        self.assertIn("initial preload; non-driven axes must still coincide", case_prompt)
        self.assertIn("Optional axial_visual binds one imported static mesh", case_prompt)
        self.assertIn("physics.collision_required=false", case_prompt)
