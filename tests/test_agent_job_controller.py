...
class SuccessfulHarness:
    ...
    def compile(self, case_spec, **kwargs):
        ...
        requested_passes = tuple(kwargs.get("render_passes") or ())
        declared_passes = tuple(
            str(modality)
            for modality in (case_spec.data.get("observation_requirements") or {}).get("modalities") or []
        )
        passes = requested_passes or declared_passes
        self.compile_calls.append((requested_views, requested_passes))
...
class AgentJobControllerTests(unittest.TestCase):
    ...
    def test_required_text_image_seed_does_not_require_planning_upload(self) -> None:
        controller, fake = self.controller()
        request = self._image_request(
            text="use the pictured geometry as the simulated object",
            required=True,
        ...
        controller.create(
            request,
            job_id="job_planning_image_seed",
            publication_tier="local_preview",
            seed_case_spec=case_spec_v2_fixture(),
        )

        inspection = controller.advance_until_blocked("job_planning_image_seed")

        self.assertEqual(inspection["job"]["state"], "awaiting_semantic_review")
        self.assertEqual(inspection["job"]["current_stage"], "semantic_review")
        self.assertFalse(inspection["job"]["authorizations"]["planning_llm_upload"])
    ...
    def test_requested_smoke_boundary_stops_before_candidate_and_can_advance(self) -> None:
        controller, fake = self.controller()
        controller.create(
            self.request,
            job_id="job_requested_smoke_boundary",
            publication_tier="local_preview",
            seed_case_spec=case_spec_v2_fixture(),
        )

        boundary = controller.advance_until_blocked(
            "job_requested_smoke_boundary",
            stop_after_stage="smoke",
        )

        self.assertEqual(boundary["job"]["state"], "blocked")
        self.assertEqual(boundary["job"]["current_stage"], "candidate")
        self.assertEqual(
            boundary["job"]["blocker"],
            {
                "code": "requested_stage_boundary_reached",
                "message": "requested stop after completed stage smoke",
                "stage": "smoke",
            },
        )
        self.assertEqual(boundary["job"]["allowed_next_actions"], ["advance", "cancel"])
        self.assertEqual(fake.execute_calls, ["smoke"])
        attempt_root = Path(boundary["paths"]["job_root"]) / "attempts" / "attempt_001"
        self.assertEqual(read_json(attempt_root / "smoke_gate.json")["status"], "pass")
        self.assertFalse((attempt_root / "candidate_run.json").exists())

        completed = controller.advance_until_blocked("job_requested_smoke_boundary")

        self.assertEqual(completed["job"]["state"], "awaiting_semantic_review")
        self.assertEqual(fake.execute_calls, ["smoke", "local_preview"])
    ...
    def test_provenance_release_override_does_not_block_publication_boundary(self) -> None:
        controller, fake = self.controller()
        controller.create(
            self.request,
            job_id="job_m4_publication_override",
            publication_tier="reference",
            ignore_provenance_and_release_gates=True,
            seed_case_spec=case_spec_v2_fixture(),
        )
        boundary = controller.advance_until_blocked("job_m4_publication_override")
        self.assertEqual(boundary["job"]["state"], "awaiting_semantic_review")
        attempt_dir = Path(boundary["paths"]["job_root"]) / "attempts" / "attempt_001"
        candidate = read_json(attempt_dir / "candidate_run.json")
        policy = read_json(Path(candidate["run_dir"]) / "quality_policy.json")
        self.assertTrue(policy["ignore_provenance_and_release_gates"])

        completed = controller.run_semantic_review("job_m4_publication_override")
    ...
    def test_exhausted_semantic_revision_budget_can_be_extended_before_revision(self) -> None:
        controller, _ = self.controller(SuccessfulHarness(semantic_status="uncertain"))
        controller.create(
            self.request,
            job_id="job_m4_semantic_revision_budget",
            publication_tier="local_preview",
            seed_case_spec=case_spec_v2_fixture(),
            budget={"max_case_spec_revisions": 1},
        )
        controller.advance_until_blocked("job_m4_semantic_revision_budget")
        blocked = controller.run_semantic_review("job_m4_semantic_revision_budget")
        self.assertEqual(blocked["job"]["allowed_next_actions"], ["cancel"])

        extended = controller.resume(
            "job_m4_semantic_revision_budget",
            max_case_spec_revisions=2,
        )

        self.assertEqual(extended["job"]["blocker"]["code"], "semantic_review_uncertain")
        self.assertEqual(extended["job"]["allowed_next_actions"], ["resume_with_revision", "cancel"])
        revised = case_spec_v2_fixture()
        revised["scene"]["duration_s"] += 0.25

        resumed = controller.resume(
            "job_m4_semantic_revision_budget",
            revised_case_spec=revised,
            revision_reason="user approved a second revision",
        )

        self.assertEqual(resumed["job"]["budget"]["max_case_spec_revisions"], 2)
        self.assertEqual(resumed["job"]["usage"]["case_spec_revisions"], 2)

    def test_exhausted_semantic_revision_budget_can_be_extended_with_revision(self) -> None:
        controller, _ = self.controller(SuccessfulHarness(semantic_status="uncertain"))
        controller.create(
            self.request,
            job_id="job_m4_combined_revision_budget",
            publication_tier="local_preview",
            seed_case_spec=case_spec_v2_fixture(),
            budget={"max_case_spec_revisions": 1},
        )
        controller.advance_until_blocked("job_m4_combined_revision_budget")
        blocked = controller.run_semantic_review("job_m4_combined_revision_budget")
        self.assertEqual(blocked["job"]["allowed_next_actions"], ["cancel"])
        revised = case_spec_v2_fixture()
        revised["scene"]["duration_s"] += 0.25

        resumed = controller.resume(
            "job_m4_combined_revision_budget",
            max_case_spec_revisions=2,
            revised_case_spec=revised,
            revision_reason="user approved a second revision",
        )
    ...
    def test_map_blocker_recompile_archives_transaction_and_preserves_provider_and_usage(self) -> None:
        ...
        compile_count_before_reopen = fake.compile_calls.count(((), ()))
        ...
        self.assertEqual(fake.compile_calls.count(((), ())), compile_count_before_reopen)

        resumed = controller.resume("job_map_config_recompile")
        self.assertEqual(resumed["job"]["state"], "awaiting_semantic_review")
        self.assertEqual(resumed["job"]["current_attempt_id"], "attempt_001")
        self.assertGreater(fake.compile_calls.count(((), ())), compile_count_before_reopen)
    ...
    def test_ue_launch_budget_stops_before_backend_invocation(self) -> None:
        ...
        self.assertEqual(inspection["job"]["allowed_next_actions"], ["resume", "inspect_artifacts", "cancel"])
        self.assertEqual(fake.execute_calls, [])

        with self.assertRaisesRegex(JobStoreError, "requires max_ue_launches"):
            controller.resume("job_ue_budget")
        with self.assertRaisesRegex(ValueError, "must exceed"):
            controller.resume("job_ue_budget", max_ue_launches=0)

        resumed = controller.resume("job_ue_budget", max_ue_launches=2)

        amendment = read_json(Path(resumed["paths"]["job_root"]) / "request" / "authorization_amendment_001.json")
        self.assertEqual(amendment["budget_changes"]["max_ue_launches"], {"before": 0, "after": 2})
        self.assertEqual(resumed["job"]["budget"]["max_ue_launches"], 2)
        self.assertEqual(resumed["job"]["usage"]["ue_launches"], 2)
        self.assertEqual(fake.execute_calls, ["smoke", "local_preview"])
    ...
    def test_revision_budget_exhaustion_preserves_trigger_and_blocks_stale_revision(self) -> None:
        ...
        self.assertEqual(
            inspection["job"]["allowed_next_actions"],
            ["resume", "resume_with_revision", "inspect_artifacts", "cancel"],
        )
        ...
        self.assertEqual(
            rejected["job"]["allowed_next_actions"],
            ["resume", "resume_with_revision", "inspect_artifacts", "cancel"],
        )
    ...
    def test_five_revision_budget_is_a_hard_limit(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "must exceed"):
            controller.resume(
                ...
                revised_case_spec=over_limit,
                revision_reason="unchanged limit",
            )

        extended = controller.resume(
            "job_revision_budget",
            max_case_spec_revisions=6,
        )

        amendment = read_json(
            Path(extended["paths"]["job_root"]) / "request" / "authorization_amendment_001.json"
        )
        self.assertEqual(
            amendment["budget_changes"]["max_case_spec_revisions"],
            {"before": 5, "after": 6},
        )
        self.assertEqual(extended["job"]["blocker"]["code"], "case_spec_revision_required")
        self.assertEqual(extended["job"]["allowed_next_actions"], ["resume_with_revision", "cancel"])
        self.assertEqual(extended["job"]["budget"]["max_case_spec_revisions"], 6)
        self.assertEqual(extended["job"]["usage"]["case_spec_revisions"], 5)
        self.assertEqual(len(extended["attempts"]), 5)

        resumed = controller.resume(
            "job_revision_budget",
            revised_case_spec=over_limit,
            revision_reason="user approved a sixth revision",
        )

        self.assertEqual(resumed["job"]["budget"]["max_case_spec_revisions"], 6)
        self.assertEqual(resumed["job"]["usage"]["case_spec_revisions"], 6)
        self.assertEqual(len(resumed["attempts"]), 6)
    ...
    def test_case_spec_revision_creates_new_immutable_attempt(self) -> None:
        ...
        self.assertEqual(resumed["job"]["state"], "blocked")
        self.assertEqual(resumed["job"]["current_stage"], "candidate")
        self.assertEqual(resumed["job"]["blocker"]["code"], "requested_stage_boundary_reached")
        ...
        completed = controller.advance_until_blocked("job_revision_history")
    ...
    def test_case_spec_revision_policy_opens_bounded_layout_fields_by_semantics(self) -> None:
        ...
        cue["physics"]["material"]["density_kg_m3"] = 1200.0
        cue["solver"] = {
            "motion": {
                "start_time_s": 0.2,
                "duration_s": 0.8,
                "pivot_local_m": [0.0, 0.0, 0.1],
        ...
        case_spec["constraints"] = [
            {
                ...
                "linear_limit_m": 0.1,
                "unilateral_distance_spring": {"rest_length_m": 0.8, "stiffness_n_m": 45.0, "damping_n_s_m": 3.0},
                "angular_limits_deg": {"x": 45.0},
        ...
        self.assertIn("$.scene.duration_s", policy["paths"])
        self.assertIn("$.observation_requirements.cameras", policy["paths"])
        ...
        self.assertIn("$.objects.cue_ball.physics.material.density_kg_m3", policy["paths"])
        self.assertIn("$.objects.floor.geometry.approx_size_m", policy["paths"])
        self.assertIn("$.objects.cue_ball.geometry.approx_size_m", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.motion.start_time_s", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.motion.duration_s", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.motion.pivot_local_m", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.motion.solver_end_rotation_xyz_deg", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.motion.ue_end_rotation_pyr_deg", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.frame_a.position_m", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.frame_a.primary_axis", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.frame_b.secondary_axis", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.linear_limit_m", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.unilateral_distance_spring.rest_length_m", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.unilateral_distance_spring.stiffness_n_m", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.unilateral_distance_spring.damping_n_s_m", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.angular_limits_deg.x", policy["paths"])
        self.assertIn("$.constraints.cue_hinge.axial_visual.mesh_forward_axis", policy["paths"])
        ...
        self.assertEqual(
            policy["ranges"]["$.constraints.cue_hinge.axial_visual.mesh_forward_axis"],
            {"kind": "enum", "values": ["x", "y", "z"]},
        )
        AgentJobController._validate_revision_changes(
            [
                {
                    "path": "$.constraints.cue_hinge.axial_visual.mesh_forward_axis",
                    "operation": "replace",
                    "before": "x",
                    "after": "z",
                }
    ...
    def test_articulated_revision_policy_opens_motion_lists_without_opening_solver_identity(self) -> None:
        case_spec = case_spec_v2_fixture()
        case_spec["objects"][0]["solver"] = {
            ...
            "root_transform_source": {
                ...
                "keyframes": [
                    ...
                    {"time_s": 2.0, "position_offset_m": [0.5, 0.0, 0.0], "rotation_offset_deg": [0.0, 0.0, 0.0]},
                ],
            },
            "ik_targets": [],
            "attachments": [
        ...
        self.assertIn("$.objects.cue_ball.solver.pose_source.keyframes", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.root_transform_source.keyframes", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.ik_targets", policy["paths"])
        self.assertIn("$.objects.cue_ball.solver.attachments", policy["paths"])
        self.assertNotIn("$.objects.cue_ball.solver.model", policy["paths"])
        self.assertNotIn("$.objects.cue_ball.solver.mode", policy["paths"])
...
class AgentJobCliTests(unittest.TestCase):
    def test_resume_cli_exposes_paid_submission_limit(self) -> None:
        ...
        self.assertIn("--max-ue-launches", completed.stdout)
        self.assertIn("--max-case-spec-revisions", completed.stdout)
        ...
        self.assertIn("--stop-after-stage", completed.stdout)

    def test_advance_cli_exposes_stop_after_stage(self) -> None:
        script = Path(__file__).resolve().parents[1] / "scripts" / "harness_agent_job.py"
        completed = subprocess.run(
            [sys.executable, str(script), "advance-until-blocked", "--help"],
            text=True,
            capture_output=True,
            check=False,
        )
