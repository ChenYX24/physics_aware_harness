...
from harness.runtime.articulated_body import (
    ARTICULATED_BODY_BONES,
    ARTICULATED_BODY_CONTROL_RIG_PATH,
    ARTICULATED_BODY_PHYSICS_ASSET_PATH,
...
# Runtime-only state for the fixed ControlRigComponent lifecycle.  The key is
# the live component path, so editor and PIE copies each receive their own
# mapping and command state.
ARTICULATED_CONTROL_RIG_CONTEXTS = {}
ARTICULATED_ANIMATION_SOURCE_TAG = "HarnessArticulatedAnimationSource"
...
RENDER_DEPTH_PASSES = os.environ.get("RENDER_DEPTH_PASSES", "1") != "0"
RENDER_SEGMENTATION_PASSES = os.environ.get("RENDER_SEGMENTATION_PASSES", "1") != "0"
RENDER_DATA_SAMPLE_COUNT = env_int("RENDER_DATA_SAMPLE_COUNT", 0, 0, 120)
...
def runtime_position(obj: dict, time_s: float, duration: float) -> tuple[list[float], list[float]]:
    ...
    pitch = 0.0
    roll = 0.0
    if behavior in {"articulated_kinematic", "articulated_ragdoll", "articulated_character"}:
        pose = sample_articulated_body_contract(params.get("articulated_body") or {}, time_s)
        position_offset = pose["root_position_offset_m"]
        rotation_offset = pose["root_rotation_offset_deg"]
        x += float(position_offset[0])
        y += float(position_offset[1])
        z += float(position_offset[2])
        pitch, yaw, roll = [float(value) for value in rotation_offset]
    elif behavior == "wind_drift":
...
def simulate_runtime_scene(runtime_scene: dict) -> list[dict]:
    ...
    for frame_idx in range(frame_count + 1):
        ...
        for obj in runtime_scene.get("dynamic_objects") or []:
            ...
            articulated_pose = (
                sample_articulated_body_contract((obj.get("params") or {}).get("articulated_body") or {}, t)
                if obj.get("behavior") in {"articulated_kinematic", "articulated_ragdoll", "articulated_character"}
                else None
            )
            frame["objects"][obj["id"]] = {
                ...
                **({"joint_rotations_deg": articulated_pose["joint_rotations_deg"]} if articulated_pose else {}),
            }
...
def actor_runtime_component(actor):
    for attr in ("static_mesh_component", "skeletal_mesh_component"):
        ...
        if component:
            return component
    for cls_name in ("PoseableMeshComponent", "GeometryCollectionComponent", "StaticMeshComponent", "SkeletalMeshComponent"):
...
def runtime_constraint_binding_details(runtime_scene: dict) -> dict:
    ...
    for binding in runtime_scene.get("constraints") or []:
        ...
        details[constraint_id] = {
            ...
            **({"unilateral_distance_spring": binding["unilateral_distance_spring"]} if binding.get("unilateral_distance_spring") is not None else {}),
            "angular_motion": binding["angular_motion"],
            "angular_limits_deg": binding["angular_limits_deg"],
            **({"linear_drive": binding["linear_drive"]} if binding.get("linear_drive") is not None else {}),
            **({"angular_drive": binding["angular_drive"]} if binding.get("angular_drive") is not None else {}),
            **({"break_thresholds": binding["break_thresholds"]} if binding.get("break_thresholds") is not None else {}),
            **({"axial_visual": binding["axial_visual"]} if binding.get("axial_visual") is not None else {}),
...
def spawn_runtime_constraints_in_game_world(actors: dict, runtime_scene: dict, status: dict) -> bool:
    ...
    for binding in bindings:
        ...
        try:
            ...
            distance_spring = binding.get("unilateral_distance_spring") or {}
            linear_drive = binding.get("linear_drive") or {}
            linear_position_enabled = linear_drive.get("position_enabled") or {}
            linear_velocity_enabled = linear_drive.get("velocity_enabled") or {}
            angular_drive = binding.get("angular_drive") or {}
            angular_position_enabled = angular_drive.get("position_enabled") or {}
            angular_velocity_enabled = angular_drive.get("velocity_enabled") or {}
            break_thresholds = binding.get("break_thresholds") or {}
            axial_visual = binding.get("axial_visual") or {}
            actor = binder(
                ...
                bool(distance_spring),
                float(distance_spring.get("rest_length_m") or 0.0) * 100.0,
                float(distance_spring.get("stiffness_n_m") or 0.0),
                float(distance_spring.get("damping_n_s_m") or 0.0),
                ...
                unreal.Vector(*[1.0 if linear_position_enabled.get(axis) else 0.0 for axis in ("x", "y", "z")]),
                unreal.Vector(*[1.0 if linear_velocity_enabled.get(axis) else 0.0 for axis in ("x", "y", "z")]),
                unreal.Vector(*[float(value) * 100.0 for value in (linear_drive.get("position_target_m") or [0.0, 0.0, 0.0])]),
                unreal.Vector(*[float(value) * 100.0 for value in (linear_drive.get("velocity_target_m_s") or [0.0, 0.0, 0.0])]),
                unreal.Vector(*[float(value) for value in (linear_drive.get("stiffness_n_m") or [0.0, 0.0, 0.0])]),
                unreal.Vector(*[float(value) for value in (linear_drive.get("damping_n_s_m") or [0.0, 0.0, 0.0])]),
                unreal.Vector(*[float(value) * 100.0 for value in (linear_drive.get("max_force_n") or [0.0, 0.0, 0.0])]),
                bool(linear_drive.get("acceleration_mode", False)),
                ue_name(angular_drive.get("mode") or "none"),
                unreal.Vector(
                    1.0 if angular_position_enabled.get("twist") else 0.0,
                    1.0 if angular_position_enabled.get("swing") else 0.0,
                    1.0 if angular_position_enabled.get("slerp") else 0.0,
                ),
                unreal.Vector(
                    1.0 if angular_velocity_enabled.get("twist") else 0.0,
                    1.0 if angular_velocity_enabled.get("swing") else 0.0,
                    1.0 if angular_velocity_enabled.get("slerp") else 0.0,
                ),
                unreal.Vector(*[float(value) for value in (angular_drive.get("orientation_target_deg") or [0.0, 0.0, 0.0])]),
                unreal.Vector(*[float(value) / (2.0 * math.pi) for value in (angular_drive.get("velocity_target_rad_s") or [0.0, 0.0, 0.0])]),
                float(angular_drive.get("stiffness_n_m_per_rad") or 0.0) * 10000.0,
                float(angular_drive.get("damping_n_m_s_per_rad") or 0.0) * 10000.0,
                float(angular_drive.get("max_torque_n_m") or 0.0) * 10000.0,
                bool(angular_drive.get("acceleration_mode", False)),
                float(break_thresholds.get("linear_force_n") or -0.01) * 100.0,
                float(break_thresholds.get("angular_torque_n_m") or -0.0001) * 10000.0,
                ue_name(axial_visual.get("object_id") or "None"),
                ue_name(axial_visual.get("mesh_forward_axis") or "x"),
                bool(binding["collision_enabled"]),
            )
            if not actor:
                raise RuntimeError("Driver rejected constraint binding or readback")
            if axial_visual:
                getter = getattr(driver, "get_constraint_visual_component", None)
                if not callable(getter):
                    raise RuntimeError("ADPPhysicsRuntime Driver constraint visual readback is unavailable")
                visual_component = getter(ue_name(constraint_id))
                if not visual_component:
                    raise RuntimeError("Driver did not expose the configured constraint visual")
                visual_object_id = str(axial_visual["object_id"])
                separate_visual_actor = (actors.get("visual_actors") or {}).get(visual_object_id)
                separate_visual_component = actor_runtime_component(separate_visual_actor) if separate_visual_actor else None
                if separate_visual_component:
                    for material_index in range(separate_visual_component.get_num_materials()):
                        visual_component.set_material(material_index, separate_visual_component.get_material(material_index))
                    separate_visual_actor.set_actor_hidden_in_game(True)
                    try:
                        separate_visual_actor.set_is_temporarily_hidden_in_editor(True)
                    except Exception:
                        pass
                    separate_visual_component.set_visibility(False, True)
                    separate_visual_component.set_hidden_in_game(True, True)
                actors.setdefault("constraint_visual_components", {})[visual_object_id] = visual_component
...
def actor_skeletal_component_with_socket(actor, socket_name: str):
    ...
    for component in components:
        try:
            if component.component_has_tag(unreal.Name(ARTICULATED_ANIMATION_SOURCE_TAG)):
                continue
        except Exception:
            pass
    ...
    for component in visible_components:
        try:
            ...
            pass
    return next(iter(visible_components), None) or actor_runtime_component(actor)


def articulated_animation_source_component(actor):
    try:
        components = actor.get_components_by_class(unreal.SkeletalMeshComponent)
    except Exception:
        components = []
...
def sync_runtime_visuals(actors: dict) -> None:
    for actor_id, visual in (actors.get("visual_actors") or {}).items():
        ...
        try:
            ...
            pass
    for object_id in (actors.get("constraint_visual_components") or {}):
        separate_visual_actor = (actors.get("visual_actors") or {}).get(object_id)
        separate_visual_component = actor_runtime_component(separate_visual_actor) if separate_visual_actor else None
        if separate_visual_actor:
            separate_visual_actor.set_actor_hidden_in_game(True)
            try:
                separate_visual_actor.set_is_temporarily_hidden_in_editor(True)
            except Exception:
        ...
        if separate_visual_component:
            ...
            separate_visual_component.set_hidden_in_game(True, True)
    driver = actors.get("adp_physics_runtime_driver")
    refresh_constraint_visuals = getattr(driver, "refresh_constraint_visuals", None) if driver else None
    if callable(refresh_constraint_visuals):
        refresh_constraint_visuals()
...
def articulated_control_rig_context(component) -> dict:
    key = str(component.get_path_name())
    context = ARTICULATED_CONTROL_RIG_CONTEXTS.get(key)
    if context is None:
        raise RuntimeError(f"articulated Control Rig lifecycle context is missing: {key}")
    return context


def articulated_control_layers_declared(contract: dict) -> bool:
    return bool(
        contract.get("pose_overlay")
        or contract.get("ik_targets")
        or contract.get("head_look_target")
    )


def articulated_control_rig_pre_initialize(component) -> None:
    context = articulated_control_rig_context(component)
    component.clear_mapped_elements()
    component.add_mapped_complete_skeletal_mesh(
        context["skeletal"],
        unreal.ControlRigComponentMapDirection.OUTPUT,
    )
    context["pre_initialize_count"] = int(context.get("pre_initialize_count") or 0) + 1


def articulated_control_rotation(transform) -> list[float]:
    rotation = unreal.MathLibrary.quat_rotator(transform.rotation)
    return [float(rotation.pitch), float(rotation.yaw), float(rotation.roll)]


def articulated_quaternion_angular_error_deg(left, right) -> float:
    left_values = [float(left.x), float(left.y), float(left.z), float(left.w)]
    right_values = [float(right.x), float(right.y), float(right.z), float(right.w)]
    dot = abs(sum(left_values[index] * right_values[index] for index in range(4)))
    norm = math.sqrt(
...
def articulated_control_key(name: str):
    return unreal.RigElementKey(
        type=unreal.RigElementType.CONTROL,
        name=unreal.Name(str(name)),
    )


def align_articulated_control_rig_component(rig_component, skeletal_component) -> None:
    rig_component.set_relative_location(
        skeletal_component.get_editor_property("relative_location"),
        False,
        False,
    )
    rig_component.set_relative_rotation(
        skeletal_component.get_editor_property("relative_rotation"),
        False,
        False,
    )
    rig_component.set_relative_scale3d(
        skeletal_component.get_editor_property("relative_scale3d")
    )


def articulated_rig_position_from_world(component, position):
    relative = unreal.MathLibrary.make_relative_transform(
        unreal.Transform(
            location=position,
            rotation=runtime_rotator([0.0, 0.0, 0.0]),
...
def articulated_world_position_from_rig(component, position):
...
def articulated_control_rig_pre_forwards_solve(component) -> None:
    context = articulated_control_rig_context(component)
    command = context.get("pending_command")
    if not isinstance(command, dict):
        return
    control_rig = component.get_control_rig()
    hierarchy = control_rig.get_hierarchy() if control_rig else None
    if not hierarchy:
        raise RuntimeError("fixed articulated Control Rig hierarchy is unavailable")
    rig_space = unreal.ControlRigComponentSpace.RIG_SPACE
    world_space = unreal.ControlRigComponentSpace.WORLD_SPACE
    state = {
        "serial": command["serial"],
        "sample_time_s": command["sample_time_s"],
        "sample_phase": "post_forwards_solve",
        "pre_initialize_executed": int(context.get("pre_initialize_count") or 0) > 0,
        "backwards_solve_executed": True,
        "pre_forwards_solve_executed": True,
        "forwards_solve_executed": False,
        "post_forwards_solve_executed": False,
        "control_write_verified": True,
        "mesh_output_sampled": False,
        "coordinate_space": "control_rig_component_lifecycle",
        "pose_overlay": [dict(result) for result in command.get("pose_overlay_inputs") or []],
        "ik": {},
        "head_look": None,
    }
    state["control_write_verified"] = all(
        float(result.get("bone_input_error_deg") or 0.0) <= 1e-3
        for result in state["pose_overlay"]
    )
    for goal, target in (command.get("ik_targets") or {}).items():
        controls = ARTICULATED_IK_CONTROLS[str(goal)]
        weight = float(target.get("weight") or 0.0)
        hierarchy.set_control_value(
            articulated_control_key(controls["switch"]),
            unreal.RigHierarchy.make_control_value_from_bool(weight > 0.0),
            unreal.RigControlValueType.CURRENT,
            False,
        )
        target_world = unreal.Transform(
            location=ue_vec_from_meters(
                target.get("position_m") or [0.0, 0.0, 0.0],
                z_offset_cm=0.0,
                origin=command["scene_origin"],
            ),
            rotation=runtime_rotator(target.get("rotation_deg") or [0.0, 0.0, 0.0]),
            scale=unreal.Vector(1.0, 1.0, 1.0),
        )
        component.set_control_transform(
            unreal.Name(controls["target"]),
            target_world,
            world_space,
        )
        pole_position = target.get("pole_position_m")
        if pole_position is not None:
            pole_world = ue_vec_from_meters(
                pole_position,
                z_offset_cm=0.0,
                origin=command["scene_origin"],
            )
            component.set_control_position(
                unreal.Name(controls["pole"]),
                articulated_rig_position_from_world(component, pole_world),
                rig_space,
            )
        switch_enabled = bool(component.get_control_bool(unreal.Name(controls["switch"])))
        target_transform = component.get_control_transform(unreal.Name(controls["target"]), world_space)
        target_location = target_transform.translation
        commanded = list(target.get("position_m") or [0.0, 0.0, 0.0])
        observed = [
            (float(target_location.x) - command["scene_origin"].x) / 100.0,
            (float(target_location.y) - command["scene_origin"].y) / 100.0,
            (float(target_location.z) - command["scene_origin"].z) / 100.0,
        ]
        target_error = math.sqrt(sum((observed[index] - float(commanded[index])) ** 2 for index in range(3)))
        result = {
            "target_control": controls["target"],
            "pole_control": controls["pole"] if pole_position is not None else None,
            "weight": weight,
            "commanded_position_m": commanded,
            "observed_switch_value": 1.0 if switch_enabled else 0.0,
            "observed_target_position_m": [round(value, 6) for value in observed],
            "target_write_error_m": round(target_error, 6),
        }
        if pole_position is not None:
            pole_rig = component.get_control_position(unreal.Name(controls["pole"]), rig_space)
            observed_pole_world = articulated_world_position_from_rig(component, pole_rig)
            observed_pole = [
                (float(observed_pole_world.x) - command["scene_origin"].x) / 100.0,
                (float(observed_pole_world.y) - command["scene_origin"].y) / 100.0,
                (float(observed_pole_world.z) - command["scene_origin"].z) / 100.0,
            ]
            result["commanded_pole_position_m"] = list(pole_position)
            result["observed_pole_position_m"] = [round(value, 6) for value in observed_pole]
            pole_error = math.sqrt(sum((observed_pole[index] - float(pole_position[index])) ** 2 for index in range(3)))
            result["pole_write_error_m"] = round(pole_error, 6)
            state["control_write_verified"] = state["control_write_verified"] and pole_error <= 1e-4
        state["control_write_verified"] = state["control_write_verified"] and (
            switch_enabled == (weight > 0.0) and target_error <= 1e-4
        )
        state["ik"][str(goal)] = result
    head_look = command.get("head_look")
    if head_look:
        weight = float(head_look.get("weight") or 0.0)
        target = ue_vec_from_meters(
            head_look.get("position_m") or [0.0, 0.0, 0.0],
            z_offset_cm=0.0,
            origin=command["scene_origin"],
        )
        head_world = component.get_bone_transform(
            unreal.Name("head"),
            unreal.ControlRigComponentSpace.WORLD_SPACE,
        )
        head_position = head_world.translation
        desired_rotation = unreal.MathLibrary.find_look_at_rotation(head_position, target)
        current_rotation = articulated_control_rotation(head_world)
        blended_rotation = runtime_rotator([
            current_rotation[0] + (((desired_rotation.pitch - current_rotation[0] + 180.0) % 360.0) - 180.0) * weight,
            current_rotation[1] + (((desired_rotation.yaw - current_rotation[1] + 180.0) % 360.0) - 180.0) * weight,
            current_rotation[2] + (((desired_rotation.roll - current_rotation[2] + 180.0) % 360.0) - 180.0) * weight,
        ])
        control = ARTICULATED_HEAD_LOOK_CONTROLS["target"]
        switch = ARTICULATED_HEAD_LOOK_CONTROLS["switch"]
        switch_enabled = weight > 0.0
        hierarchy.set_control_value(
            articulated_control_key(switch),
            unreal.RigHierarchy.make_control_value_from_bool(switch_enabled),
            unreal.RigControlValueType.CURRENT,
            False,
        )
        current_control_world = component.get_control_transform(unreal.Name(control), world_space)
        control_from_head = unreal.MathLibrary.make_relative_transform(current_control_world, head_world)
        desired_head_world = unreal.Transform(
            location=head_position,
            rotation=blended_rotation,
            scale=head_world.scale3d,
        )
        target_control_world = unreal.MathLibrary.compose_transforms(control_from_head, desired_head_world)
        component.set_control_transform(
            unreal.Name(control),
            target_control_world,
            world_space,
        )
        observed_switch = bool(component.get_control_bool(unreal.Name(switch)))
        observed_control = component.get_control_transform(unreal.Name(control), world_space)
        observed_position = observed_control.translation
        target_position_values = (
            float(target_control_world.translation.x),
            float(target_control_world.translation.y),
            float(target_control_world.translation.z),
        )
        position_error = math.sqrt(sum(
            (value - target_position_values[index]) ** 2
            for index, value in enumerate((float(observed_position.x), float(observed_position.y), float(observed_position.z)))
        ))
        observed_rotation = articulated_control_rotation(observed_control)
        target_rotation = articulated_control_rotation(target_control_world)
        rotation_error = articulated_quaternion_angular_error_deg(
            observed_control.rotation,
            target_control_world.rotation,
        )
        state["control_write_verified"] = state["control_write_verified"] and (
            observed_switch == switch_enabled
        ...
        state["head_look"] = {
            ...
            "weight": weight,
            "observed_switch_enabled": observed_switch,
            "control_write_position_error_cm": round(position_error, 6),
            "control_write_rotation_error_deg": round(rotation_error, 6),
            "target_control_world_position_cm": [
                float(target_control_world.translation.x),
                float(target_control_world.translation.y),
                float(target_control_world.translation.z),
            ],
            "target_control_world_rotation_deg": target_rotation,
        }
    context["active_state"] = state
    context["pre_forwards_serial"] = command["serial"]


def articulated_control_rig_post_forwards_solve(component) -> None:
    context = articulated_control_rig_context(component)
    command = context.get("pending_command")
    state = context.get("active_state")
    if not isinstance(command, dict) or not isinstance(state, dict):
        return
    if state.get("serial") != command.get("serial"):
        return
    rig_space = unreal.ControlRigComponentSpace.RIG_SPACE
    world_space = unreal.ControlRigComponentSpace.WORLD_SPACE
    state["forwards_solve_executed"] = True
    state["post_forwards_solve_executed"] = True
    for result in state["pose_overlay"]:
        actual_bone = articulated_control_rotation(
            component.get_bone_transform(
                unreal.Name(result["bone"]),
                unreal.ControlRigComponentSpace.LOCAL_SPACE,
            )
        )
        result["post_solve_bone_local_rotation_deg"] = actual_bone
    for goal, result in state["ik"].items():
        controls = ARTICULATED_IK_CONTROLS[goal]
        switch_enabled = bool(component.get_control_bool(unreal.Name(controls["switch"])))
        switch_value = 1.0 if switch_enabled else 0.0
        target_transform = component.get_control_transform(unreal.Name(controls["target"]), world_space)
        target_location = target_transform.translation
        commanded = result["commanded_position_m"]
        observed = [
            (float(target_location.x) - command["scene_origin"].x) / 100.0,
            (float(target_location.y) - command["scene_origin"].y) / 100.0,
            (float(target_location.z) - command["scene_origin"].z) / 100.0,
        ...
        target_error = math.sqrt(sum((observed[index] - float(commanded[index])) ** 2 for index in range(3)))
        result["post_solve_switch_value"] = round(switch_value, 6)
        result["post_solve_target_position_m"] = [round(value, 6) for value in observed]
        result["post_solve_target_delta_m"] = round(target_error, 6)
        if result.get("pole_control"):
            pole_rig = component.get_control_position(unreal.Name(result["pole_control"]), rig_space)
            pole_world = articulated_world_position_from_rig(component, pole_rig)
            result["post_solve_pole_position_m"] = [
                round((float(pole_world.x) - command["scene_origin"].x) / 100.0, 6),
                round((float(pole_world.y) - command["scene_origin"].y) / 100.0, 6),
                round((float(pole_world.z) - command["scene_origin"].z) / 100.0, 6),
            ]
    if state.get("head_look"):
        result = state["head_look"]
        switch_enabled = bool(component.get_control_bool(unreal.Name(result["switch_control"])))
        observed_head = component.get_control_transform(unreal.Name(result["control"]), world_space)
        observed_position = observed_head.translation
        observed_position_values = (float(observed_position.x), float(observed_position.y), float(observed_position.z))
        target_position_values = result["target_control_world_position_cm"]
        position_error = math.sqrt(sum(
            (value - target_position_values[index]) ** 2
            for index, value in enumerate(observed_position_values)
        ))
        observed_rotation = articulated_control_rotation(observed_head)
        target_rotation = result["target_control_world_rotation_deg"]
        rotation_error = max(
            abs(((observed_rotation[index] - target_rotation[index] + 180.0) % 360.0) - 180.0)
            for index in range(3)
        )
        result["post_solve_control_position_delta_cm"] = round(position_error, 6)
        result["post_solve_control_rotation_delta_deg"] = round(rotation_error, 6)
        result["post_solve_switch_enabled"] = switch_enabled
    context["completed_state"] = state
    context["post_forwards_serial"] = command["serial"]


def initialize_articulated_control_rig_component(
    rig_component,
    skeletal_component,
    ...
    align_articulated_control_rig_component(rig_component, skeletal_component)
    context = {
        "skeletal": skeletal_component,
        "animation_source": animation_source_component,
        "pre_initialize_count": 0,
        "command_serial": 0,
        "pending_command": None,
        "active_state": None,
        "completed_state": None,
    }
    ARTICULATED_CONTROL_RIG_CONTEXTS[str(rig_component.get_path_name())] = context
    rig_component.on_pre_initialize_delegate.add_callable_unique(articulated_control_rig_pre_initialize)
    rig_component.on_pre_forwards_solve_delegate.add_callable_unique(articulated_control_rig_pre_forwards_solve)
    rig_component.on_post_forwards_solve_delegate.add_callable_unique(articulated_control_rig_post_forwards_solve)
    if rig_class is not None:
        rig_component.set_control_rig_class(rig_class)
    rig_component.set_bone_initial_transforms_from_skeletal_mesh(skeletal_mesh)
    rig_component.initialize()
    pre_initialize_count = int(context.get("pre_initialize_count") or 0)
    if pre_initialize_count < 1 or not rig_component.can_execute():
        raise RuntimeError(
            "fixed articulated Control Rig initialization failed: "
            f"pre_initialize_count={pre_initialize_count}, can_execute={bool(rig_component.can_execute())}"
        )
    rig_component.update(0.0)


def add_articulated_animation_source(actor, skeletal_component, skeletal_mesh):
    subobjects = unreal.get_engine_subsystem(unreal.SubobjectDataSubsystem)
    parent_handles = subobjects.k2_gather_subobject_data_for_instance(actor)
    if not parent_handles:
        raise RuntimeError("articulated Character has no subobject root handle")
    component_handle, fail_reason = subobjects.add_new_subobject(
    ...
    component_data = subobjects.k2_find_subobject_data_from_handle(component_handle)
    source = (
        unreal.SubobjectDataBlueprintFunctionLibrary.get_associated_object(component_data)
        if component_data is not None
        else None
    )
    if not source:
        raise RuntimeError(f"failed to add articulated animation source: {fail_reason}")
    source.set_skeletal_mesh(skeletal_mesh)
    source.set_mobility(unreal.ComponentMobility.MOVABLE)
    source.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION)
    source.set_relative_location(skeletal_component.get_editor_property("relative_location"), False, False)
    source.set_relative_rotation(skeletal_component.get_editor_property("relative_rotation"), False, False)
    source.set_relative_scale3d(skeletal_component.get_editor_property("relative_scale3d"))
    source.set_editor_property("component_tags", [unreal.Name(ARTICULATED_ANIMATION_SOURCE_TAG)])
    source.set_editor_property(
        "visibility_based_anim_tick_option",
        unreal.VisibilityBasedAnimTickOption.ALWAYS_TICK_POSE_AND_REFRESH_BONES,
    )
    source.set_visibility(False, True)
    source.set_hidden_in_game(True, True)
    return source


def add_articulated_control_rig(actor, skeletal_component, animation_source_component, skeletal_mesh):
    subobjects = unreal.get_engine_subsystem(unreal.SubobjectDataSubsystem)
    parent_handles = subobjects.k2_gather_subobject_data_for_instance(actor)
    if not parent_handles:
        raise RuntimeError("articulated Character has no subobject root handle")
    component_handle, fail_reason = subobjects.add_new_subobject(
        unreal.AddNewSubobjectParams(
            parent_handle=parent_handles[0],
            new_class=unreal.ControlRigComponent,
            conform_transform_to_parent=True,
        )
    )
    component_data = subobjects.k2_find_subobject_data_from_handle(component_handle)
    ...
    if not rig_component:
        raise RuntimeError(f"failed to add fixed ControlRigComponent: {fail_reason}")
    rig_asset = load_asset(ARTICULATED_BODY_CONTROL_RIG_PATH)
    rig_component.set_editor_property("update_rig_on_tick", False)
    rig_component.set_editor_property("update_in_editor", False)
    rig_component.set_editor_property("reset_transform_before_tick", False)
    initialize_articulated_control_rig_component(
        rig_component,
        skeletal_component,
        animation_source_component,
        skeletal_mesh,
        rig_asset.generated_class(),
    )
    return rig_component
...
def spawn_runtime_actor(
    ...
    rotation=None,
    enable_control_rig: bool = False,
):
    subsystem = unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
    asset_kind = str(asset_kind or "").lower()
    if asset_kind == "articulated_character":
        asset = load_asset(asset_path)
        actor = subsystem.spawn_actor_from_class(unreal.Character, location)
        if not actor:
            raise RuntimeError(f"failed to spawn articulated character: {asset_path}")
        actor.set_actor_label(label)
        ...
        if not component or not capsule or not movement:
            raise RuntimeError(f"articulated character components are incomplete: {asset_path}")
        component.set_skeletal_mesh(asset)
        component.set_mobility(unreal.ComponentMobility.MOVABLE)
        component.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION)
        component.set_relative_location(unreal.Vector(0.0, 0.0, -96.0), False, False)
        component.set_relative_rotation(runtime_rotator([0.0, -90.0, 0.0]), False, False)
        actor.cache_initial_mesh_offset(
        ...
        capsule.set_capsule_size(29.0, 96.0, True)
        capsule.set_collision_profile_name("Pawn")
        movement.set_editor_property("max_walk_speed", 100.0)
        movement.set_editor_property("max_acceleration", 200.0)
        white = create_generated_material(
            "M_Harness_ArticulatedBodyWhite",
            unreal.LinearColor(0.82, 0.82, 0.82, 1.0),
            0.72,
            0.0,
            0.0,
        )
        for material_index in range(max(1, int(component.get_num_materials()))):
            component.set_material(material_index, white)
        if enable_control_rig:
            animation_source = add_articulated_animation_source(actor, component, asset)
            add_articulated_control_rig(actor, component, animation_source, asset)
        if rotation:
            actor.set_actor_rotation(rotation, False)
        return actor
    if asset_kind == "articulated_kinematic":
        asset = load_asset(asset_path)
        actor = subsystem.spawn_actor_from_class(unreal.Actor, location)
        if not actor:
            raise RuntimeError(f"failed to spawn articulated actor: {asset_path}")
        actor.set_actor_label(label)
        try:
            subobjects = unreal.get_engine_subsystem(unreal.SubobjectDataSubsystem)
            parent_handles = subobjects.k2_gather_subobject_data_for_instance(actor)
            if not parent_handles:
                raise RuntimeError("actor instance has no subobject root handle")
            component_handle, fail_reason = subobjects.add_new_subobject(
                unreal.AddNewSubobjectParams(
                    parent_handle=parent_handles[0],
            ...
            if not component:
                raise RuntimeError(f"failed to add PoseableMeshComponent: {fail_reason}")
            component.set_skeletal_mesh(asset)
            component.set_mobility(unreal.ComponentMobility.MOVABLE)
            component.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION)
            component.set_world_scale3d(scale)
        except Exception as exc:
            raise RuntimeError(f"failed to create PoseableMeshComponent for {label}: {exc}") from exc
        white = create_generated_material(
            "M_Harness_ArticulatedBodyWhite",
            unreal.LinearColor(0.82, 0.82, 0.82, 1.0),
            0.72,
        ...
        if rotation:
            actor.set_actor_rotation(rotation, False)
        return actor
    if asset_kind == "articulated_ragdoll":
        asset = load_asset(asset_path)
        actor = subsystem.spawn_actor_from_class(unreal.SkeletalMeshActor, location)
        if not actor:
            raise RuntimeError(f"failed to spawn articulated ragdoll: {asset_path}")
        actor.set_actor_label(label)
        component = actor_runtime_component(actor)
        if not component:
            raise RuntimeError(f"missing articulated ragdoll component: {asset_path}")
        component.set_skeletal_mesh(asset)
        physics_asset = load_asset(ARTICULATED_BODY_PHYSICS_ASSET_PATH)
        component.set_physics_asset(physics_asset, False)
        component.set_mobility(unreal.ComponentMobility.MOVABLE)
        component.set_world_scale3d(scale)
        white = create_generated_material(
            "M_Harness_ArticulatedBodyWhite",
...
def configure_runtime_physics(actor, obj: dict, role: str, controls: dict | None = None) -> dict:
    ...
    if properties.get("simulate_physics") is False or properties.get("simulate_physics") in ("force_off", "force_off_until_release", "disabled"):
        role_simulate = False
...
def runtime_animation_segments(obj: dict) -> list[dict]:
    params = obj.get("params") if isinstance(obj.get("params"), dict) else {}
    articulated = params.get("articulated_body") if isinstance(params.get("articulated_body"), dict) else {}
    pose_source = articulated.get("pose_source") if isinstance(articulated.get("pose_source"), dict) else {}
    if pose_source.get("type") == "animation_sequence":
        return [
            {
                "name": str(segment.get("animation_asset_id") or ""),
                "start_s": segment.get("start_time_s"),
                "end_s": segment.get("end_time_s"),
                "animation_ref": segment.get("animation_asset_path"),
                "loop": segment.get("loop") is True,
                "time_scale": segment.get("play_rate"),
            }
            for segment in pose_source.get("segments") or []
...
def play_runtime_animation(
    ...
    *,
    loop: bool,
    local_time_s: float,
    root_transform_source_type: str,
    status: dict,
) -> dict:
    detail = {
        "id": actor_id,
        ...
        "played": False,
        "position_set": False,
        "root_transform_source_type": root_transform_source_type,
        "errors": [],
    }
    component = (
        articulated_animation_source_component(actor)
        or actor_skeletal_component_with_socket(actor, "root")
    )
    ...
    if anim_instance:
        root_motion_mode_name = (
            "ROOT_MOTION_FROM_EVERYTHING"
            if root_transform_source_type == "animation_root_motion"
            else "NO_ROOT_MOTION_EXTRACTION"
        )
        root_motion_mode = getattr(getattr(unreal, "RootMotionMode", None), root_motion_mode_name, None)
        if root_motion_mode is not None:
            try:
                anim_instance.set_root_motion_mode(root_motion_mode)
                detail["root_motion_mode"] = root_motion_mode_name
            except Exception as exc:
                detail["errors"].append(f"root_motion_mode:{exc}")
    if not detail["played"]:
        detail["errors"].extend(play_errors[-3:])
    clip_duration_s = float(anim.get_editor_property("sequence_length"))
    detail["clip_duration_s"] = round(clip_duration_s, 6)
    sample_time_s = max(0.0, float(local_time_s))
    if loop and clip_duration_s > 0.0:
        sample_time_s %= clip_duration_s
    position_method = getattr(component, "set_position", None)
    if position_method:
        for args in ((sample_time_s, False), (sample_time_s,)):
            try:
                position_method(*args)
                detail["position_set"] = True
                detail["sample_time_s"] = round(sample_time_s, 6)
                break
            except Exception as exc:
...
def apply_runtime_animation_segments(
    ...
    for obj in runtime_scene.get("dynamic_objects") or []:
        ...
        if is_gasp:
            ...
            articulated = (obj.get("params") or {}).get("articulated_body") or {}
            root_source = articulated.get("root_transform_source") or {}
            detail = play_runtime_animation(
                ...
                root_transform_source_type=str(root_source.get("type") or "root_keyframes"),
                status=status,
            )
...
def component_bounds(component) -> tuple[unreal.Vector, unreal.Vector]:
...
def runtime_visual_scale(obj: dict, visual_path: str) -> list[float] | None:
    params = obj.get("params") if isinstance(obj.get("params"), dict) else {}
    scale = params.get("intact_visual_scale") or params.get("visual_instance_scale")
    if scale is None and visual_path == str(obj.get("ue5_path") or ""):
        scale = obj.get("scale")
    if not isinstance(scale, list) or len(scale) < 3:
        return None
    return [float(value) for value in scale[:3]]
...
def align_runtime_actor_to_pose_anchor(
    ...
    """Apply the compiled pose registration for a runtime actor."""
    params = obj.get("params") if isinstance(obj.get("params"), dict) else {}
    pose_anchor = params.get("pose_anchor")
    if pose_anchor == "solver_to_visual":
        registration = params.get("solver_to_visual")
        if not isinstance(registration, dict):
            raise RuntimeError(f"runtime solver-to-visual registration is missing for {obj.get('id')}")
        translation_m = registration.get("translation_m")
        solver_rotation = registration.get("solver_rotation_xyz_deg")
        ue_rotation = registration.get("ue_rotation_pyr_deg")
        if not all(isinstance(value, list) and len(value) == 3 for value in (translation_m, solver_rotation, ue_rotation)):
            raise RuntimeError(f"runtime solver-to-visual registration is invalid for {obj.get('id')}")
        numeric = [float(value) for values in (translation_m, solver_rotation, ue_rotation) for value in values]
        if not all(math.isfinite(value) for value in numeric):
            raise RuntimeError(f"runtime solver-to-visual registration is non-finite for {obj.get('id')}")
        expected_ue_rotation = [-numeric[4], -numeric[5], numeric[3]]
        if any(abs(float(ue_rotation[index]) - expected_ue_rotation[index]) > 1e-6 for index in range(3)):
            raise RuntimeError(f"runtime solver-to-visual rotation conventions disagree for {obj.get('id')}")
        before_location = actor.get_actor_location()
        solver_world_rotation = actor.get_actor_rotation()
        expected_rotation = unreal.MathLibrary.compose_rotators(
            runtime_rotator(ue_rotation),
            solver_world_rotation,
        )
        local_translation_cm = unreal.Vector(*[float(value) * 100.0 for value in translation_m])
        world_translation_cm = unreal.MathLibrary.transform_direction(
            actor.get_actor_transform(),
            local_translation_cm,
        )
        expected_location = desired_position + world_translation_cm
        ...
        residual = math.sqrt(
            (actual_location.x - expected_location.x) ** 2
            + (actual_location.y - expected_location.y) ** 2
            + (actual_location.z - expected_location.z) ** 2
        )
        if residual > 0.1:
            raise RuntimeError(
                f"runtime pose registration failed for {obj.get('id')}: "
                f"solver-to-visual residual {residual:.6f} cm"
            )
        rotation_residual = max(
            abs((actual - expected + 180.0) % 360.0 - 180.0)
            for actual, expected in (
                (actual_rotation.pitch, expected_rotation.pitch),
                (actual_rotation.yaw, expected_rotation.yaw),
                (actual_rotation.roll, expected_rotation.roll),
            )
        )
        if rotation_residual > 0.01:
            raise RuntimeError(
                f"runtime pose registration failed for {obj.get('id')}: "
                f"solver-to-visual rotation residual {rotation_residual:.6f} degrees"
            )
        if registrations is not None:
            ...
            registrations[object_id] = {
                ...
                "solver_position_cm": [desired_position.x, desired_position.y, desired_position.z],
                "declared_solver_to_visual": registration,
                "expected_visual_position_cm": [expected_location.x, expected_location.y, expected_location.z],
                "actual_visual_position_cm": [actual_location.x, actual_location.y, actual_location.z],
                ...
                "max_rotation_residual_deg": max(
                    float(current.get("max_rotation_residual_deg") or 0.0),
                    rotation_residual,
                ),
            }
        return actor_bounds(actor)
    if pose_anchor != "bounds_center":
        return actor_bounds(actor)
    target_position = desired_position
    local_offset_m = params.get("visual_local_bounds_center_from_collision_m")
    if isinstance(local_offset_m, list) and len(local_offset_m) >= 3:
        local_offset_cm = unreal.Vector(*[float(value) * 100.0 for value in local_offset_m[:3]])
        target_position = desired_position + unreal.MathLibrary.transform_direction(
            actor.get_actor_transform(),
            local_offset_cm,
        )
    before_origin, extent = actor_bounds(actor)
    before_location = actor.get_actor_location()
    correction = target_position - before_origin
    actor.set_actor_location(before_location + correction, False, False)
    after_origin, after_extent = actor_bounds(actor)
    residual = math.sqrt(
        (after_origin.x - target_position.x) ** 2
        + (after_origin.y - target_position.y) ** 2
        + (after_origin.z - target_position.z) ** 2
    )
    if residual > 0.1:
        raise RuntimeError(
            f"runtime pose registration failed for {obj.get('id')}: "
    ...
    if registrations is not None:
        ...
        registrations[object_id] = {
            ...
            "desired_position_cm": [target_position.x, target_position.y, target_position.z],
...
def create_translucent_fluid_material(color, roughness: float, opacity: float, refraction: float):
    package_path = "/Game/AgenticGenerated/Materials"
    asset_name = f"M_Harness_UnifiedFluidTranslucent_V1_{GENERATED_MATERIAL_VERSION}"
    ...
    try:
        ...
        expressions = [
            (unreal.MaterialExpressionConstant3Vector, "constant", color, unreal.MaterialProperty.MP_BASE_COLOR, -160),
            (unreal.MaterialExpressionConstant, "r", roughness, unreal.MaterialProperty.MP_ROUGHNESS, 40),
            (unreal.MaterialExpressionConstant, "r", opacity, unreal.MaterialProperty.MP_OPACITY, 140),
            (unreal.MaterialExpressionConstant, "r", refraction, unreal.MaterialProperty.MP_REFRACTION, 240),
        ]
        for expression_class, property_name, value, material_property, y in expressions:
            node = unreal.MaterialEditingLibrary.create_material_expression(material, expression_class, -420, y)
...
def disable_runtime_stage_helper_collision(actor) -> dict:
    component = actor_runtime_component(actor)
    if not component:
        raise RuntimeError(f"readability stage helper has no primitive component: {actor.get_actor_label()}")
    actor.set_actor_enable_collision(False)
    component.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION)
    component.set_collision_profile_name("NoCollision")
    component.set_simulate_physics(False)
    set_editor_property_if_available(component, "generate_overlap_events", False)
    if bool(actor.get_actor_enable_collision()):
        raise RuntimeError(f"readability stage helper collision remained enabled: {actor.get_actor_label()}")
    return {
...
def spawn_runtime_stage_helpers(editor, runtime_scene: dict, scene_origin: unreal.Vector, materials: dict) -> list[dict]:
    ...
    def helper(label: str, loc: tuple[float, float, float], scale: tuple[float, float, float], material, rot: tuple[float, float, float] | None = None):
        ...
        collision_audit = disable_runtime_stage_helper_collision(actor)
        set_actor_material(actor, material)
        helpers.append({
            "label": label,
            "mesh": "/Engine/BasicShapes/Cube.Cube",
...
def setup_scene(runtime_scene: dict | None = None):
    ...
    def place_runtime(obj, loc, rotation_degrees=(0.0, 0.0, 0.0)):
        ...
        articulated_contract = ((obj.get("params") or {}).get("articulated_body") or {})
        enable_control_rig = articulated_control_layers_declared(articulated_contract)
        ...
        if params.get("generate_translucent_fluid_material") is True:
            material = create_translucent_fluid_material(
                runtime_object_color(obj, unreal.LinearColor(0.03, 0.28, 0.48, 1.0)),
                float(params["roughness"]),
                float(params["opacity"]),
                float(params["refraction"]),
            )
            if not material:
                raise RuntimeError(f"failed to create translucent fluid material: {obj.get('id')}")
            set_actor_material(actor, material)
            material_override = True
        elif params.get("generate_solid_material") is True:
            material = create_generated_material(
    ...
    if runtime_scene:
        ...
        def add_runtime_visual(obj: dict, physics_actor) -> None:
            ...
            if not visual_path:
                return
            try:
                visual_scale = runtime_visual_scale(obj, visual_path)
                spawn_scale = (
                    unreal.Vector(*visual_scale)
                ...
                if visual_scale is None and not (obj.get("params") or {}).get("preserve_visual_authored_scale"):
        ...
        def runtime_binding_snapshot_for(
            obj: dict,
            physics_actor,
            physics_detail: dict,
            constraint_visual_components: dict,
        ) -> dict:
            object_id = str(obj.get("id") or "")
            params = obj.get("params") if isinstance(obj.get("params"), dict) else {}
            properties = obj.get("physics_properties") if isinstance(obj.get("physics_properties"), dict) else {}
            ...
            articulated_contract = params.get("articulated_body") if isinstance(params.get("articulated_body"), dict) else None
            render_component = constraint_visual_components.get(object_id)
            if not render_component and articulated_contract:
                render_component = actor_skeletal_component_with_socket(render_actor, "root")
            if not render_component:
                render_component = actor_runtime_component(render_actor)
            visibility_actor = render_actor
            try:
                visibility_actor = render_component.get_owner() or render_actor
            except Exception:
                pass
            collision_component = actor_runtime_component(physics_actor)
            ...
            try:
                actor_hidden = bool(visibility_actor.get_actor_hidden_in_game())
            except Exception:
                try:
                    actor_hidden = bool(visibility_actor.get_editor_property("hidden"))
                except Exception:
                    actor_hidden = False
            try:
            ...
            return {
                ...
                "mesh_component": {
                    "actor_class": unreal_class_name(render_actor),
                    "component_class": unreal_class_name(render_component),
                ...
                "articulated_components": (
                    {
                        "capsule_registered": runtime_component_registered(
                            physics_actor.get_component_by_class(unreal.CapsuleComponent)
                        ),
                        "character_movement_registered": runtime_component_registered(
                            physics_actor.get_component_by_class(unreal.CharacterMovementComponent)
                        ),
                        "control_rig_registered": runtime_component_registered(
                            physics_actor.get_component_by_class(unreal.ControlRigComponent)
                        ),
                        "control_rig_path": (
                            ((obj.get("params") or {}).get("articulated_body") or {}).get("control_rig_path")
                            if runtime_component_registered(
                                physics_actor.get_component_by_class(unreal.ControlRigComponent)
                            )
                            else None
                        ),
                    }
                    if str(obj.get("behavior") or "") == "articulated_character"
                    else None
                ),
            ...
            return {
                ...
                "objects": [
                    runtime_binding_snapshot_for(
                        ...
                        actor_bindings.get("constraint_visual_components") or {},
                    )
                    for obj in [
...
def data_capture_frame_indices(frame_count: int) -> set[int]:
    if frame_count <= 0:
        return set()
    if RENDER_DATA_SAMPLE_COUNT <= 0 or RENDER_DATA_SAMPLE_COUNT >= frame_count:
        return set(range(frame_count))
    return {
        round(slot * (frame_count - 1) / max(1, RENDER_DATA_SAMPLE_COUNT - 1))
        for slot in range(RENDER_DATA_SAMPLE_COUNT)
    }


def should_capture_data_frame(frame_index: int, frame_count: int) -> bool:
    return frame_index in data_capture_frame_indices(frame_count)
...
def export_depth_and_segmentation_frame(
    ...
    if not RENDER_DEPTH_PASSES:
        result["depth"] = {"status": "disabled", "path": None, "source_type": None}
        segmentation = export_instance_segmentation_frame(
            actors,
            runtime_scene,
            view_id,
            frame_index,
        ...
        if segmentation.get("status") != "available" and RENDER_SEGMENTATION_PASSES:
    ...
    result["segmentation"] = export_instance_segmentation_frame(
        actors,
        runtime_scene,
        view_id,
        frame_index,
    ...
    if result["segmentation"].get("status") != "available" and RENDER_SEGMENTATION_PASSES:
        result["errors"].append("segmentation_capture_failed")

    return result


def export_instance_segmentation_frame(
    actors: dict,
    runtime_scene: dict | None,
    view_id: str,
    frame_index: int,
    ...
    if not RENDER_SEGMENTATION_PASSES:
        return {"status": "disabled", "path": None, "source_type": None, "instance_count": 0}
    capture_comp = actors["capture_comp"]
    original_source = capture_comp.get_editor_property("capture_source")
    original_target = capture_comp.get_editor_property("texture_target")
    ...
    return {"status": "missing", "path": None, "source_type": None, "instance_count": 0}
...
def set_capture_view(actors: dict, location: unreal.Vector, target: unreal.Vector, fov: float) -> None:
    ...
    actors["capture"].set_actor_rotation(rotation, False)
    actors["capture_comp"].set_editor_property("fov_angle", float(fov))
    actors["capture_component_audit"] = capture_component_audit(
        actors["capture_comp"], actors, STUDIO_RUNTIME_SCENE
    )


def capture_component_audit(component, actors: dict, runtime_scene: dict | None) -> dict:
    location = component.get_world_location()
    rotation = component.get_world_rotation()
    forward = component.get_forward_vector()
    targets = []
    for obj in (runtime_scene or {}).get("dynamic_objects", []) + (runtime_scene or {}).get("static_objects", []):
        object_id = str(obj.get("id") or "")
        actor = (actors.get("visual_actors") or {}).get(object_id) or actors.get(object_id)
        if not object_id or not actor:
            continue
        actor_location = actor.get_actor_location()
        delta = actor_location - location
        targets.append(
            {
                "object_id": object_id,
                "position_cm": vector_payload(actor_location),
                "camera_forward_depth_cm": round(
                    float(delta.x * forward.x + delta.y * forward.y + delta.z * forward.z),
                    4,
                ),
            }
        )
    return {
        ...
        "forward": vector_payload(forward),
        "targets": targets,
    }
...
def compiled_camera_view_specs(runtime_scene: dict, scene_origin: unreal.Vector) -> list[dict]:
    camera = runtime_scene.get("camera") if isinstance((runtime_scene or {}).get("camera"), dict) else {}
    planned = [item for item in camera.get("views") or [] if isinstance(item, dict) and item.get("camera_id")]
    if not planned:
        raise RuntimeError("compiled camera plan contains no views")
    result: list[dict] = []
    for index, spec in enumerate(planned):
        ...
        if len(location) != 3 or len(planned_target) != 3:
            raise RuntimeError(f"compiled camera view has an invalid pose: {camera_id}")
        result.append(
            {
                "id": camera_id,
                "view_id": camera_id,
                "suffix": "" if index == 0 else f"_{camera_id}",
                "label": str(spec.get("role") or camera_id),
                "location": ue_vec_from_meters(location, z_offset_cm=0.0, origin=scene_origin),
                "target": ue_vec_from_meters(planned_target, z_offset_cm=0.0, origin=scene_origin),
                "fov": float(spec.get("fov") or 60.0),
                "near_clip": spec.get("near_clip"),
                "far_clip": spec.get("far_clip"),
                "camera_mode": str(spec.get("camera_mode") or "fixed"),
                "dynamic_camera_profile": spec.get("dynamic_camera_profile"),
                "subject_follow_location_gain": float(
                    spec["subject_follow_location_gain"]
                    if spec.get("subject_follow_location_gain") is not None
                    else 1.0
                ),
                "subject_follow_target_gain": float(
                    spec["subject_follow_target_gain"]
...
def camera_view_specs(actors: dict, runtime_scene: dict | None) -> list[dict]:
    if runtime_scene:
        scene_origin = unreal.Vector(*actors.get("scene_origin", [0.0, 0.0, 0.0]))
        return compiled_camera_view_specs(runtime_scene, scene_origin)
...
def articulated_runtime_ids(runtime_scene: dict | None) -> list[str]:
    return [
        str(obj.get("id") or "")
        for obj in (runtime_scene or {}).get("dynamic_objects") or []
        if str(obj.get("behavior") or "") in {"articulated_kinematic", "articulated_ragdoll", "articulated_character"}
    ]


def articulated_frame_zero_needs_post_tick(runtime_scene: dict | None) -> bool:
    return any(
        str(obj.get("behavior") or "") == "articulated_character"
        for obj in (runtime_scene or {}).get("dynamic_objects") or []
    )


def apply_articulated_character_command(actors: dict, actor, obj: dict, frame_obj: dict) -> None:
    contract = (obj.get("params") or {}).get("articulated_body") or {}
    root_source = contract.get("root_transform_source") or {}
    source_type = str(root_source.get("type") or "")
    movement = actor.get_component_by_class(unreal.CharacterMovementComponent)
    if not movement:
        raise RuntimeError(f"articulated character movement component is missing: {obj.get('id')}")
    if source_type == "character_movement":
        max_speed_cm_s = float(root_source.get("max_speed_m_s") or 1.0) * 100.0
        max_acceleration_cm_s2 = float(root_source.get("max_acceleration_m_s2") or 2.0) * 100.0
        movement.set_editor_property("max_walk_speed", max_speed_cm_s)
        movement.set_editor_property("max_acceleration", max_acceleration_cm_s2)
        desired = frame_obj.get("position") or obj.get("initial_position_m") or [0.0, 0.0, 0.0]
        scene_origin = unreal.Vector(*actors.get("scene_origin", [0.0, 0.0, 0.0]))
        desired_world = ue_vec_from_meters(desired, origin=scene_origin)
        current = actor.get_actor_location()
        delta = unreal.Vector(desired_world.x - current.x, desired_world.y - current.y, 0.0)
        length = math.hypot(delta.x, delta.y)
        command_state = actors.setdefault("articulated_character_movement_state", {}).setdefault(
            str(obj.get("id") or ""),
            {"velocity_cm_s": [0.0, 0.0, 0.0]},
        )
        commanded_velocity = command_state.get("velocity_cm_s") or [0.0, 0.0, 0.0]
        movement.velocity = unreal.Vector(
            float(commanded_velocity[0]),
            float(commanded_velocity[1]),
            float(commanded_velocity[2]),
        )
        if length > 0.01:
            actor.add_movement_input(unreal.Vector(delta.x / length, delta.y / length, 0.0), 1.0, True)
        rotation = frame_obj.get("rotation_degrees") or [0.0, 0.0, 0.0]
        actor.set_actor_rotation(runtime_rotator(rotation), False)
    elif source_type != "animation_root_motion":
        raise RuntimeError(f"unsupported articulated character root source: {source_type}")


def apply_articulated_pose(actors: dict, obj: dict, frame_obj: dict, time_s: float) -> bool:
    object_id = str(obj.get("id") or "")
    actor = actors.get(object_id)
    if not actor:
        raise RuntimeError(f"articulated actor is missing: {object_id}")
    contract = (obj.get("params") or {}).get("articulated_body") or {}
    component = actor_runtime_component(actor)
    if not component:
        raise RuntimeError(f"articulated mesh component is missing: {object_id}")
    ragdoll_start = contract.get("ragdoll_start_time_s")
    if contract.get("mode") == "ragdoll" and ragdoll_start is not None and time_s >= float(ragdoll_start):
        state = actors.setdefault("articulated_ragdoll_state", {})
        if not state.get(object_id):
            component.set_collision_enabled(unreal.CollisionEnabled.QUERY_AND_PHYSICS)
            component.set_collision_profile_name("PhysicsActor")
            component.set_simulate_physics(True)
            component.set_enable_gravity(True)
            component.wake_all_rigid_bodies()
            state[object_id] = True
        return False
    rotations = frame_obj.get("joint_rotations_deg") or {}
    if not rotations:
        return True
    setter = getattr(component, "set_bone_rotation_by_name", None)
    getter = getattr(component, "get_bone_rotation_by_name", None)
    if not setter or not getter:
        if any(any(abs(float(value)) > 1e-9 for value in rotation) for rotation in rotations.values()):
            raise RuntimeError(f"articulated pose component cannot set declared bones: {object_id}")
        return True
    references = actors.setdefault("articulated_reference_rotations", {}).setdefault(object_id, {})
    for bone, offset in rotations.items():
        if bone not in references:
            base = getter(unreal.Name(str(bone)), unreal.BoneSpaces.COMPONENT_SPACE)
            references[bone] = [float(base.pitch), float(base.yaw), float(base.roll)]
        base = references[bone]
        setter(
            unreal.Name(str(bone)),
            runtime_rotator([base[index] + float(offset[index]) for index in range(3)]),
            unreal.BoneSpaces.COMPONENT_SPACE,
        )
    return True


def seed_control_rig_from_animation(rig, animation_source, overlay: dict | None) -> list[dict]:
    rig_space = unreal.ControlRigComponentSpace.RIG_SPACE
    local_space = unreal.ControlRigComponentSpace.LOCAL_SPACE
    for bone_index in range(int(animation_source.get_num_bones())):
        bone_name = animation_source.get_bone_name(bone_index)
        rig.set_bone_transform(
            bone_name,
            animation_source.get_bone_transform(
                bone_name,
    ...
    overlay_inputs = []
    if overlay:
        weight = float(overlay.get("weight") or 0.0)
        for bone, rotation in (overlay.get("rotations_deg") or {}).items():
            bone_name = unreal.Name(str(bone))
            base_local = rig.get_bone_transform(bone_name, local_space)
            offset = [float(value) * weight for value in rotation]
            offset_transform = unreal.Transform(
                location=unreal.Vector(0.0, 0.0, 0.0),
                rotation=runtime_rotator(offset),
                scale=unreal.Vector(1.0, 1.0, 1.0),
            )
            target_local = unreal.MathLibrary.compose_transforms(offset_transform, base_local)
            rig.set_bone_transform(bone_name, target_local, local_space, 1.0, True)
            observed_local = rig.get_bone_transform(bone_name, local_space)
            observed_offset = unreal.MathLibrary.make_relative_transform(observed_local, base_local)
            observed_offset_rotation = articulated_control_rotation(observed_offset)
            input_error = max(
                abs(((observed_offset_rotation[index] - offset[index] + 180.0) % 360.0) - 180.0)
                for index in range(3)
            )
            overlay_inputs.append({
                "bone": str(bone),
                "weight": weight,
                "commanded_rotation_offset_deg": offset,
                "observed_bone_local_offset_deg": observed_offset_rotation,
                "bone_input_error_deg": round(input_error, 6),
            })
    control_rig = rig.get_control_rig()
    if not control_rig or not control_rig.execute(unreal.Name("Backwards Solve")):
        raise RuntimeError("fixed articulated Control Rig cannot execute Backwards Solve")
    return overlay_inputs


def finalize_articulated_control_output(rig, state: dict) -> None:
    context = articulated_control_rig_context(rig)
    skeletal = context["skeletal"]
    animation_source = context["animation_source"]
    output_bones = {result["bone"] for result in state["pose_overlay"]} | set(state["ik"])
    if state.get("head_look"):
        output_bones.add("head")
    state["mesh_output_bones"] = {}
    for bone in output_bones:
        transform = skeletal.get_socket_transform(
            unreal.Name(str(bone)),
            unreal.RelativeTransformSpace.RTS_WORLD,
        )
        state["mesh_output_bones"][str(bone)] = {
            "position_cm": [
                float(transform.translation.x),
                float(transform.translation.y),
                float(transform.translation.z),
            ],
            "rotation_deg": articulated_control_rotation(transform),
        }
    for result in state["pose_overlay"]:
        bone_name = unreal.Name(result["bone"])
        before_local = animation_source.get_bone_transform(
            bone_name,
            unreal.RelativeTransformSpace.RTS_PARENT_BONE_SPACE,
        )
        after_local = skeletal.get_bone_transform(
            bone_name,
            unreal.RelativeTransformSpace.RTS_PARENT_BONE_SPACE,
        )
        output_offset = unreal.MathLibrary.make_relative_transform(after_local, before_local)
        result["output_rotation_delta_deg"] = [
            round(float(value), 6)
            for value in articulated_control_rotation(output_offset)
        ]
    state["mesh_output_sampled"] = bool(output_bones)


def apply_articulated_control_layers(actors: dict, obj: dict, time_s: float) -> None:
    object_id = str(obj.get("id") or "")
    actor = actors.get(object_id)
    if not actor:
        raise RuntimeError(f"articulated Control Rig actor is missing: {object_id}")
    contract = (obj.get("params") or {}).get("articulated_body") or {}
    sampled = sample_articulated_body_contract(contract, time_s)
    overlay = sampled.get("pose_overlay")
    ik_targets = sampled.get("ik_targets") or {}
    head_look = sampled.get("head_look_target")
    if not overlay and not ik_targets and not head_look:
        return
    rig = actor.get_component_by_class(unreal.ControlRigComponent)
    skeletal = actor_skeletal_component_with_socket(actor, "head")
    if not rig or not skeletal:
        raise RuntimeError(f"fixed articulated Control Rig binding is missing: {object_id}")
    context = articulated_control_rig_context(rig)
    animation_source = context.get("animation_source")
    if not animation_source:
        raise RuntimeError(f"articulated animation source is missing: {object_id}")
    overlay_inputs = seed_control_rig_from_animation(rig, animation_source, overlay)
    serial = int(context.get("command_serial") or 0) + 1
    context["command_serial"] = serial
    context["pending_command"] = {
        "serial": serial,
        "sample_time_s": round(float(time_s), 6),
        "scene_origin": unreal.Vector(*actors.get("scene_origin", [0.0, 0.0, 0.0])),
        "pose_overlay_inputs": overlay_inputs,
        "ik_targets": ik_targets,
        "head_look": head_look,
    }
    context["active_state"] = None
    context["completed_state"] = None
    rig.update(0.0)
    applied = context.get("completed_state")
    if not isinstance(applied, dict) or applied.get("serial") != serial:
        raise RuntimeError("fixed articulated Control Rig did not complete Pre/Forward/Post lifecycle")
    finalize_articulated_control_output(rig, applied)
    actors.setdefault("articulated_control_state", {})[object_id] = applied


def support_surface_height_cm(
    support_actor,
    support_obj: dict,
    x_cm: float,
    y_cm: float,
    support_binding_source: str,
) -> tuple[float, str]:
    rotation = support_actor.get_actor_rotation()
    origin, extent = actor_bounds(support_actor)
    if abs(float(rotation.pitch)) <= 1e-4 and abs(float(rotation.roll)) <= 1e-4:
        return float(origin.z + extent.z), "horizontal_actor_bounds"
    if str(support_binding_source or "") != "solver.support_object_id":
        raise RuntimeError("inclined articulated support requires explicit solver.support_object_id")
    geometry = (support_obj.get("physics_properties") or {}).get("collision_geometry") or {}
    if str(geometry.get("shape") or "") != "box":
        raise RuntimeError("inclined articulated support requires explicit box collision geometry")
    size_m = geometry.get("size_m") or []
    if not isinstance(size_m, list) or len(size_m) != 3:
        raise RuntimeError("inclined articulated support box size is unavailable")
    pitch, yaw, roll = [math.radians(value) for value in (rotation.pitch, rotation.yaw, rotation.roll)]
    cp, sp = math.cos(pitch), math.sin(pitch)
    cy, sy = math.cos(yaw), math.sin(yaw)
    cr, sr = math.cos(roll), math.sin(roll)
    normal = unreal.Vector(cy * sp * cr + sy * sr, sy * sp * cr - cy * sr, cp * cr)
    if abs(float(normal.z)) <= 1e-4:
        raise RuntimeError("articulated support surface is vertical")
    actor_location = support_actor.get_actor_location()
    top = unreal.Vector(
        actor_location.x + normal.x * float(size_m[2]) * 50.0,
        actor_location.y + normal.y * float(size_m[2]) * 50.0,
        actor_location.z + normal.z * float(size_m[2]) * 50.0,
    )
    height = top.z - (normal.x * (float(x_cm) - top.x) + normal.y * (float(y_cm) - top.y)) / normal.z
    return float(height), "explicit_inclined_box_plane"


def calibrate_articulated_grounding(actors: dict, runtime_scene: dict | None) -> None:
    objects = {
        str(obj.get("id") or ""): obj
        for obj in ((runtime_scene or {}).get("dynamic_objects") or []) + ((runtime_scene or {}).get("static_objects") or [])
    }
    records = actors.setdefault("articulated_grounding", {})
    for object_id, obj in objects.items():
        if str(obj.get("behavior") or "") != "articulated_character":
            continue
        support_id = str((obj.get("params") or {}).get("support_object_id") or "")
        if not support_id:
            records[object_id] = {"status": "not_requested"}
            continue
        actor = actors.get(object_id)
        support_actor = actors.get(support_id)
        support_obj = objects.get(support_id)
        if not actor or not support_actor or not support_obj:
            raise RuntimeError(f"articulated support binding is unavailable: {object_id}->{support_id}")
        location = actor.get_actor_location()
        binding_source = str((obj.get("params") or {}).get("support_binding_source") or "")
        support_height, surface_source = support_surface_height_cm(
            support_actor,
            support_obj,
            location.x,
            location.y,
        ...
        skeletal = actor_skeletal_component_with_socket(actor, "root")
        capsule = actor.get_component_by_class(unreal.CapsuleComponent)
        movement = actor.get_component_by_class(unreal.CharacterMovementComponent)
        if not skeletal or not capsule or not movement:
            raise RuntimeError(f"articulated grounding components are incomplete: {object_id}")
        capsule_half_height = float(capsule.get_scaled_capsule_half_height())
        final_location = unreal.Vector(location.x, location.y, support_height + capsule_half_height)
        actor.set_actor_location(final_location, False, True)
        movement.set_movement_mode(unreal.MovementMode.MOVE_WALKING)
        actor.set_actor_location(final_location, False, True)
        bounds_origin, bounds_extent = component_bounds(skeletal)
        foot_height = float(bounds_origin.z - bounds_extent.z)
        mesh_offset_cm = support_height - foot_height
        relative_location = skeletal.get_editor_property("relative_location")
        skeletal.set_relative_location(
            unreal.Vector(
                float(relative_location.x),
                float(relative_location.y),
                float(relative_location.z) + mesh_offset_cm,
            ),
            False,
            False,
        )
        actor.cache_initial_mesh_offset(
            skeletal.get_editor_property("relative_location"),
            skeletal.get_editor_property("relative_rotation"),
        )
        animation_source = articulated_animation_source_component(actor)
        if animation_source:
            source_location = animation_source.get_editor_property("relative_location")
            animation_source.set_relative_location(
                unreal.Vector(
                    float(source_location.x),
                    float(source_location.y),
                    float(source_location.z) + mesh_offset_cm,
                ),
                False,
                False,
            )
        rig = actor.get_component_by_class(unreal.ControlRigComponent)
        if rig:
            align_articulated_control_rig_component(rig, skeletal)
        calibrated_location = actor.get_actor_location()
        calibrated_rotation = actor.get_actor_rotation()
        actor_offset_cm = float(calibrated_location.z - location.z)
        actors.setdefault("runtime_ground_offsets", {})[object_id] = actor_offset_cm
        actors.setdefault("runtime_initial_transforms", {})[object_id] = {
            "position_cm": [calibrated_location.x, calibrated_location.y, calibrated_location.z],
            "rotation_degrees": [calibrated_rotation.pitch, calibrated_rotation.yaw, calibrated_rotation.roll],
        }
        records[object_id] = {
            "status": "applied",
            "support_object_id": support_id,
            "support_binding_source": (obj.get("params") or {}).get("support_binding_source"),
            "measurement_source": "evaluated_skeletal_mesh_bounds",
            "surface_source": surface_source,
            "vertical_offset_m": round(actor_offset_cm / 100.0, 6),
            "mesh_vertical_offset_m": round(mesh_offset_cm / 100.0, 6),
            "capsule_half_height_m": round(capsule_half_height / 100.0, 6),
            "movement_mode_initialized": "walking",
            "support_height_m": round((support_height - unreal.Vector(*actors.get("scene_origin", [0.0, 0.0, 0.0])).z) / 100.0, 6),
        }


def apply_articulated_attachments(actors: dict, runtime_scene: dict | None, time_s: float) -> None:
    objects = [
        obj
        for obj in (runtime_scene or {}).get("dynamic_objects") or []
        if str(obj.get("behavior") or "") in {"articulated_kinematic", "articulated_ragdoll", "articulated_character"}
    ...
    states = actors.setdefault("articulated_attachment_state", {})
    for obj in objects:
        body_actor = actors.get(str(obj.get("id") or ""))
        body_component = actor_skeletal_component_with_socket(body_actor, "root") if body_actor else None
        if not body_component:
            raise RuntimeError(f"articulated attachment source is missing: {obj.get('id')}")
        contract = (obj.get("params") or {}).get("articulated_body") or {}
        for attachment in contract.get("attachments") or []:
            target_id = str(attachment.get("object_id") or "")
            target = actors.get(target_id)
            if not target:
                raise RuntimeError(f"articulated attachment target is missing: {target_id}")
            start = float(attachment.get("start_time_s") or 0.0)
            end_value = attachment.get("end_time_s")
            active = time_s >= start and (end_value is None or time_s < float(end_value))
            target_component = actor_runtime_component(target)
            if active:
                if not states.get(target_id):
                    properties = (by_id.get(target_id) or {}).get("physics_properties") or {}
                    if target_component:
                        target_component.set_simulate_physics(False)
                        target_component.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION)
                    states[target_id] = {
                        "collision_enabled": bool_control(properties.get("collision_enabled"), True),
                        "simulate_physics": properties.get("simulate_physics") is True,
                    }
                bone = unreal.Name(str(attachment.get("bone") or ""))
                local_m = attachment.get("local_position_m") or [0.0, 0.0, 0.0]
                local_rotation = attachment.get("local_rotation_deg") or [0.0, 0.0, 0.0]
                world_position, world_rotation = body_component.transform_from_bone_space(
                    bone,
                    unreal.Vector(*[float(value) * 100.0 for value in local_m]),
                    runtime_rotator(local_rotation),
                )
                target.set_actor_location(world_position, False, False)
                target.set_actor_rotation(world_rotation, False)
            elif attachment_state := states.pop(target_id, None):
                properties = (by_id.get(target_id) or {}).get("physics_properties") or {}
                if target_component:
                    target_component.set_collision_enabled(
                        unreal.CollisionEnabled.QUERY_AND_PHYSICS
                        if attachment_state.get("collision_enabled")
                        else unreal.CollisionEnabled.NO_COLLISION
                    )
                    if attachment_state.get("simulate_physics"):
                        target_component.set_simulate_physics(True)
                        target_component.set_enable_gravity(bool_control(properties.get("enable_gravity"), True))
                        target_component.wake_all_rigid_bodies()
...
def apply_trajectory_frame(actors: dict, dynamic_ids: list[str], frame: dict, scene_origin: unreal.Vector, runtime_scene: dict | None) -> None:
    ...
    for oid in dynamic_ids:
        ...
        if str(params.get("humanoid_adapter") or "").strip().lower() in {"gasp", "ddv"}:
            ...
            continue
        articulated_obj = obj_by_id.get(oid) or {}
        contract = params.get("articulated_body") if isinstance(params.get("articulated_body"), dict) else None
        time_s = float(frame.get("time") or 0.0)
        if contract and contract.get("mode") == "ragdoll" and time_s >= float(contract.get("ragdoll_start_time_s") or 0.0):
            apply_articulated_pose(actors, articulated_obj, frame_obj, time_s)
            continue
        if contract and str(articulated_obj.get("behavior") or "") == "articulated_character":
            apply_articulated_character_command(actors, actors[oid], articulated_obj, frame_obj)
            continue
        ...
        if contract:
            apply_articulated_pose(actors, articulated_obj, frame_obj, time_s)
    apply_articulated_attachments(actors, runtime_scene, float(frame.get("time") or 0.0))
...
def rebind_constraints_after_body_physics_state_change(
    actors: dict,
    body_id: str,
    status: dict,
) -> list[str]:
    """Recreate solver handles invalidated by a constrained body's physics-state change."""
    details = actors.get("constraint_binding_details") or {}
    constraint_actors = actors.get("constraint_actors") or {}
    rebound = []
    for constraint_id, detail in details.items():
        endpoints = [detail.get("body_a") or {}, detail.get("body_b") or {}]
        if body_id not in {str(endpoint.get("object_id") or "") for endpoint in endpoints}:
            continue
        constraint_actor = constraint_actors.get(constraint_id)
        constraint_component = physics_constraint_component(constraint_actor)
        if constraint_component is None:
            raise RuntimeError(f"constraint rebind component is missing: {constraint_id}")
        components = []
        for endpoint in endpoints:
            if endpoint.get("endpoint_type") == "world_anchor":
                components.append(None)
                continue
            endpoint_id = str(endpoint.get("object_id") or "")
            endpoint_component = actor_runtime_component(actors.get(endpoint_id))
            if endpoint_component is None:
                raise RuntimeError(f"constraint rebind body is missing: {constraint_id}:{endpoint_id}")
            components.append(endpoint_component)
        try:
            constraint_component.set_constrained_components(
                components[0],
                ue_name("None"),
                components[1],
                ue_name("None"),
            ...
            raise RuntimeError(f"constraint rebind failed: {constraint_id}:{exc}") from exc
        actual = constrained_runtime_components(constraint_component)
        expected = [component for component in components if component is not None]
        if len(actual) != len(expected) or any(component not in actual for component in expected):
            raise RuntimeError(f"constraint rebind readback mismatch: {constraint_id}")
        rebound.append(str(constraint_id))
    if rebound:
        status.setdefault("delayed_release_constraint_rebinds", []).append(
            {"body_id": body_id, "constraint_ids": sorted(rebound)}
        )
...
def apply_delayed_release_projectiles(
    ...
    for obj in projectiles:
        ...
        rebind_constraints_after_body_physics_state_change(actors, str(actor_id), status)
        default_velocity = (
            (3.0, 0.0, 1.0)
...
def world_transform_payload(location, rotation, scene_origin: unreal.Vector) -> dict:
    return {
        "position_m": [
            round((float(location.x) - scene_origin.x) / 100.0, 5),
            round((float(location.y) - scene_origin.y) / 100.0, 5),
            round((float(location.z) - scene_origin.z) / 100.0, 5),
        ],
        "rotation_deg": [
            round(float(rotation.pitch), 4),
            round(float(rotation.yaw), 4),
...
def observed_articulated_state(
    actors: dict,
    actor_id: str,
    actor,
    obj: dict,
    ...
    contract = (obj.get("params") or {}).get("articulated_body") or {}
    component = actor_skeletal_component_with_socket(actor, "root")
    animation_component = articulated_animation_source_component(actor) or component
    actor_transform = world_transform_payload(actor.get_actor_location(), actor.get_actor_rotation(), scene_origin)
    observed = {
        "sample_phase": "post_tick",
        "sample_time_s": round(float(elapsed_s), 6),
        "frame_zero_evaluated": (actors.get("articulated_frame_zero_evaluation") or {}).get("status") == "ready",
        "actor_class": actor.get_class().get_name(),
        "actor_transform": actor_transform,
        "pose_source_type": str((contract.get("pose_source") or {}).get("type") or ""),
        "root_transform_source_type": str((contract.get("root_transform_source") or {}).get("type") or ""),
        "bones": {},
        "ik": {},
        "errors": [],
    }
    capsule = actor.get_component_by_class(unreal.CapsuleComponent)
    if capsule:
        observed["capsule_transform"] = world_transform_payload(
            capsule.get_world_location(), capsule.get_world_rotation(), scene_origin
        )
    movement = actor.get_component_by_class(unreal.CharacterMovementComponent)
    if movement:
        get_name = getattr(movement, "get_movement_name", None)
        try:
            observed["movement_mode"] = str(get_name() if get_name else movement.get_editor_property("movement_mode"))
        except Exception as exc:
            observed["errors"].append(f"movement_mode:{exc}")
        velocity = movement.velocity
        observed["velocity_m_s"] = [
            round(float(velocity.x) / 100.0, 5),
            round(float(velocity.y) / 100.0, 5),
            round(float(velocity.z) / 100.0, 5),
        ]
    if component:
        for bone in sorted(ARTICULATED_BODY_BONES):
            try:
                location = component.get_socket_location(unreal.Name(bone))
                rotation = component.get_socket_rotation(unreal.Name(bone))
                observed["bones"][bone] = world_transform_payload(location, rotation, scene_origin)
            except Exception as exc:
                observed["errors"].append(f"bone:{bone}:{exc}")
        get_position = getattr(animation_component, "get_position", None)
        try:
            observed["animation_time_s"] = round(float(get_position()), 5) if get_position else None
        except Exception as exc:
            observed["animation_time_s"] = None
            observed["errors"].append(f"animation_time:{exc}")
    animation_state = (actors.get("runtime_animation_state") or {}).get(actor_id) or {}
    observed["animation_asset_path"] = animation_state.get("animation_ref")
    observed["animation_segment"] = animation_state.get("segment_name")
    observed["animation_playing"] = bool(
        animation_state.get("animation_ref")
        and not (animation_state.get("last_errors") or [])
    )
    previous = actors.setdefault("articulated_observed_previous", {}).get(actor_id)
    if previous:
        observed["root_motion_delta"] = {
            "translation_m": [
                round(actor_transform["position_m"][index] - previous["position_m"][index], 6)
                for index in range(3)
            ],
            "rotation_deg": [
                round(((actor_transform["rotation_deg"][index] - previous["rotation_deg"][index] + 180.0) % 360.0) - 180.0, 6)
                for index in range(3)
            ],
        }
    else:
        observed["root_motion_delta"] = {"translation_m": [0.0, 0.0, 0.0], "rotation_deg": [0.0, 0.0, 0.0]}
    actors["articulated_observed_previous"][actor_id] = actor_transform
    sampled = sample_articulated_body_contract(contract, elapsed_s)
    rig = actor.get_component_by_class(unreal.ControlRigComponent)
    control_state = (actors.get("articulated_control_state") or {}).get(actor_id) or {}
    observed["control_rig"] = {
        "registered": runtime_component_registered(rig),
        "asset_path": contract.get("control_rig_path"),
        "evaluation_time_s": control_state.get("sample_time_s"),
        "evaluation_phase": control_state.get("sample_phase"),
        "pre_initialize_executed": control_state.get("pre_initialize_executed") is True,
        "backwards_solve_executed": control_state.get("backwards_solve_executed") is True,
        "pre_forwards_solve_executed": control_state.get("pre_forwards_solve_executed") is True,
        "forwards_solve_executed": control_state.get("forwards_solve_executed") is True,
        "post_forwards_solve_executed": control_state.get("post_forwards_solve_executed") is True,
        "control_write_verified": control_state.get("control_write_verified") is True,
        "mesh_output_sampled": control_state.get("mesh_output_sampled") is True,
        "coordinate_space": control_state.get("coordinate_space"),
    }
    overlay = sampled.get("pose_overlay")
    observed["pose_overlay"] = {"results": [], "weight": float((overlay or {}).get("weight") or 0.0)}
    if overlay and rig:
        overlay_weight = float(overlay.get("weight") or 0.0)
        for bone, rotation in (overlay.get("rotations_deg") or {}).items():
            applied_result = next(
                (item for item in control_state.get("pose_overlay") or [] if str(item.get("bone") or "") == str(bone)),
                {},
            )
            actual = applied_result.get("observed_bone_local_offset_deg") or []
            expected = applied_result.get("commanded_rotation_offset_deg") or [
                float(component) * overlay_weight for component in rotation
            ]
            error = applied_result.get("bone_input_error_deg")
            observed["pose_overlay"]["results"].append({
                "bone": bone,
                "commanded_rotation_deg": expected,
                "observed_rotation_deg": [round(value, 5) for value in actual],
                "error_deg": round(error, 6) if isinstance(error, (int, float)) else None,
                "output_rotation_delta_deg": applied_result.get("output_rotation_delta_deg"),
            })
    for goal, target in (sampled.get("ik_targets") or {}).items():
        effector = observed["bones"].get(goal)
        target_position = target.get("position_m") or [0.0, 0.0, 0.0]
        controls = ARTICULATED_IK_CONTROLS[str(goal)]
        applied_ik = (control_state.get("ik") or {}).get(str(goal)) or {}
        target_control_position = applied_ik.get("observed_target_position_m")
        switch_value = applied_ik.get("observed_switch_value")
        error_m = None
        if effector:
            error_m = round(math.sqrt(sum(
                (float(effector["position_m"][index]) - float(target_position[index])) ** 2
                for index in range(3)
            )), 6)
        observed["ik"][goal] = {
            "commanded_target_transform": {
                "position_m": target_position,
                "rotation_deg": target.get("rotation_deg") or [0.0, 0.0, 0.0],
            },
            "target_control": {
                "control": controls["target"],
                "observed_position_m": target_control_position,
            },
            "switch_control": controls["switch"],
            "switch_value": switch_value,
            "effector_transform": effector,
            "weight": target.get("weight"),
            "tolerance_m": target.get("tolerance_m"),
            "error_m": error_m,
        }
        pole_target = target.get("pole_position_m")
        if pole_target is not None:
            pole_control = ARTICULATED_IK_CONTROLS[str(goal)]["pole"]
            observed["ik"][goal]["pole"] = {
                "control": pole_control,
                "target_position_m": pole_target,
                "observed_position_m": applied_ik.get("observed_pole_position_m"),
            }
    head_look = sampled.get("head_look_target")
    observed["head_look"] = None
    if head_look and component:
        target = head_look.get("position_m") or [0.0, 0.0, 0.0]
        head = component.get_socket_location(unreal.Name("head"))
        rotation = component.get_socket_rotation(unreal.Name("head"))
        forward = unreal.MathLibrary.get_forward_vector(rotation)
        target_world = ue_vec_from_meters(target, z_offset_cm=0.0, origin=scene_origin)
        delta = unreal.Vector(target_world.x - head.x, target_world.y - head.y, target_world.z - head.z)
        length = max(1e-9, math.sqrt(delta.x * delta.x + delta.y * delta.y + delta.z * delta.z))
        dot = max(-1.0, min(1.0, (forward.x * delta.x + forward.y * delta.y + forward.z * delta.z) / length))
        applied_head = control_state.get("head_look") or {}
        observed["head_look"] = {
            "target_position_m": target,
            "weight": head_look.get("weight"),
            "tolerance_deg": head_look.get("tolerance_deg"),
            "angular_error_deg": round(math.degrees(math.acos(dot)), 6),
            "switch_control": applied_head.get("switch_control"),
            "switch_enabled": applied_head.get("observed_switch_enabled"),
        }
    grounding = (actors.get("articulated_grounding") or {}).get(actor_id)
    observed["grounding"] = dict(grounding) if isinstance(grounding, dict) else None
    if isinstance(grounding, dict) and grounding.get("status") == "applied":
        support_id = str(grounding.get("support_object_id") or "")
        support_actor = actors.get(support_id)
        all_objects = {
            str(item.get("id") or ""): item
            for item in ((runtime_scene or {}).get("dynamic_objects") or []) + ((runtime_scene or {}).get("static_objects") or [])
        }
        if support_actor and support_id in all_objects:
            location = actor.get_actor_location()
            support_height, _ = support_surface_height_cm(
                support_actor,
                all_objects[support_id],
                location.x,
                location.y,
                str(grounding.get("support_binding_source") or ""),
            )
            bounds_origin, bounds_extent = component_bounds(component)
            observed["grounding"]["support_gap_m"] = round(
                ((bounds_origin.z - bounds_extent.z) - support_height) / 100.0,
                6,
            )
    observed["ragdoll_active"] = bool((actors.get("articulated_ragdoll_state") or {}).get(actor_id))
    return observed
...
def record_physics_transform_frame(
    ...
    for actor_id in dynamic_ids:
        if actor_id in actors:
            ...
            obj = dynamic_by_id.get(actor_id) or {}
            if isinstance((obj.get("params") or {}).get("articulated_body"), dict):
                frame["objects"][actor_id]["articulated_observed"] = observed_articulated_state(
                    actors,
                    actor_id,
                    actors[actor_id],
                    obj,
...
def start_editor_physics_capture(actors: dict, runtime_scene: dict | None, expected_seconds: float) -> dict:
    ...
    try:
        world = actors.get("world") or unreal.EditorLevelLibrary.get_editor_world()
        status["manual_world_tick_available"] = callable(getattr(world, "tick", None))
    except Exception as exc:
        status["errors"].append(f"manual_world_tick_capability:{exc}")
...
def prepare_cpp_runtime_driver(actors: dict, runtime_scene: dict | None, status: dict, max_frames: int) -> bool:
    ...
    for obj in runtime_scene.get("dynamic_objects") or []:
        ...
        if not actor:
            continue
        if obj.get("behavior") in {"third_person_runner", "articulated_character"}:
            continue
        properties = obj.get("physics_properties") or {}
    ...
    for force in runtime_scene.get("forces") or []:
        force_id = str(force.get("id") or "")
        object_id = str(force.get("object") or "")
        vector_n = vector_from_sequence(force.get("vector_n"), 1.0)
        try:
            registered = bool(
                driver.register_continuous_force(
                    ue_name(force_id),
                    ue_name(object_id),
                    vector_n,
                    float(force.get("start_time_s")),
                    float(force.get("end_time_s")),
                )
            )
        except Exception as exc:
            cpp_status["errors"].append(f"register_continuous_force:{force_id}:{exc}")
            registered = False
        if not registered:
            cpp_status["errors"].append(f"register_continuous_force:{force_id}:false")
            return False
        cpp_status.setdefault("registered_forces", []).append(
            {
                "force_id": force_id,
                "object": object_id,
                "vector_n": [float(vector_n.x), float(vector_n.y), float(vector_n.z)],
                "start_time_s": float(force.get("start_time_s")),
                "end_time_s": float(force.get("end_time_s")),
            }
        )

    for contact in runtime_scene.get("compliant_contacts") or []:
        contact_id = str(contact.get("id") or "")
        body_a = str(contact.get("body_a") or "")
        body_b = str(contact.get("body_b") or "")
        try:
            registered = bool(
                driver.register_compliant_contact(
                    ue_name(contact_id),
                    ue_name(body_a),
                    ue_name(body_b),
                    float(contact.get("activation_distance_m")),
                    float(contact.get("stiffness_n_m")),
                    float(contact.get("damping_n_s_m")),
                )
            )
        except Exception as exc:
            cpp_status["errors"].append(f"register_compliant_contact:{contact_id}:{exc}")
            registered = False
        if not registered:
            cpp_status["errors"].append(f"register_compliant_contact:{contact_id}:false")
            return False
        cpp_status.setdefault("registered_compliant_contacts", []).append(
            {
                "contact_id": contact_id,
                "body_a": body_a,
                "body_b": body_b,
                "activation_distance_m": float(contact.get("activation_distance_m")),
                "stiffness_n_m": float(contact.get("stiffness_n_m")),
                "damping_n_s_m": float(contact.get("damping_n_s_m")),
            }
        )
...
def cpp_capture_to_runtime_trajectory(capture: dict, actors: dict, runtime_scene: dict | None, scene_origin: unreal.Vector) -> tuple[list[dict], list[dict], list[dict], dict]:
    ...
    for frame in capture.get("frames") or []:
        ...
        runtime_frame = {
            ...
            "constraints": [],
            "forces": [dict(force) for force in frame.get("forces") or [] if isinstance(force, dict)],
        }
        for raw_constraint in frame.get("constraints") or []:
            if not isinstance(raw_constraint, dict) or not raw_constraint.get("constraint_id"):
                continue
            translation_m = [
                round(float(value) / 100.0, 7)
                for value in (raw_constraint.get("translation_cm") or [0.0, 0.0, 0.0])[:3]
            ]
            relative_velocity_m_s = [
                round(float(value) / 100.0, 7)
                for value in (raw_constraint.get("relative_velocity_cm_s") or [0.0, 0.0, 0.0])[:3]
            ]
            position_target_m = [
                round(float(value) / 100.0, 7)
                for value in (raw_constraint.get("position_target_cm") or [0.0, 0.0, 0.0])[:3]
            ]
            stiffness_n_m = [
                float(value)
                for value in (raw_constraint.get("stiffness_n_m") or [0.0, 0.0, 0.0])[:3]
            ]
            distance_spring = (
                {
                    "rest_length_m": float(raw_constraint.get("distance_spring_rest_length_cm") or 0.0) / 100.0,
                    "stiffness_n_m": float(raw_constraint.get("distance_spring_stiffness_n_m") or 0.0),
                    "damping_n_s_m": float(raw_constraint.get("distance_spring_damping_n_s_m") or 0.0),
                    "distance_m": float(raw_constraint.get("distance_spring_distance_cm") or 0.0) / 100.0,
                    "extension_m": float(raw_constraint.get("distance_spring_extension_cm") or 0.0) / 100.0,
                    "separation_speed_m_s": float(raw_constraint.get("distance_spring_separation_speed_cm_s") or 0.0) / 100.0,
                    "tension_n": float(raw_constraint.get("distance_spring_tension_n") or 0.0),
                    "direction_a_to_b": [float(value) for value in (raw_constraint.get("distance_spring_direction_a_to_b") or [0.0, 0.0, 0.0])[:3]],
                    "force_on_body_b_n": [float(value) for value in (raw_constraint.get("distance_spring_force_on_body_b_n") or [0.0, 0.0, 0.0])[:3]],
                    "cumulative_evaluation_count": int(raw_constraint.get("distance_spring_evaluation_count") or 0),
                    "cumulative_active_evaluation_count": int(raw_constraint.get("distance_spring_active_evaluation_count") or 0),
                }
                if raw_constraint.get("unilateral_distance_spring_enabled") is True
                else None
            )
            deformation_m = [
                round(translation_m[index] - position_target_m[index], 7)
                for index in range(3)
            ]
            runtime_frame["constraints"].append(
                {
                    "constraint_id": str(raw_constraint["constraint_id"]),
                    ...
                    "linear_force_n": [
                        round(float(value) / 100.0, 7)
                        for value in (raw_constraint.get("linear_force_engine_units") or [0.0, 0.0, 0.0])[:3]
                    ],
                    "angular_torque_n_m": [
                        round(float(value) / 10000.0, 7)
                        for value in (raw_constraint.get("angular_torque_engine_units") or [0.0, 0.0, 0.0])[:3]
                    ],
                    "stiffness_n_m": stiffness_n_m,
                    **({"unilateral_distance_spring": distance_spring} if distance_spring is not None else {}),
                    "elastic_potential_j": round(
                        sum(
                            0.5 * stiffness_n_m[index] * deformation_m[index] * deformation_m[index]
                            for index in range(3)
                        ) + (
                            0.5 * float(distance_spring["stiffness_n_m"]) * float(distance_spring["extension_m"]) ** 2
                            if distance_spring is not None else 0.0
                        ),
                        8,
                    ),
                    "broken": bool(raw_constraint.get("broken")),
                    "frame": frame_index,
...
def merge_scripted_runner_trajectory(
    ...
    *,
    ...
    articulated_ids = [
        ...
        for obj in runtime_scene.get("dynamic_objects") or []
        if obj.get("id")
        and obj.get("behavior") in {"articulated_kinematic", "articulated_ragdoll", "articulated_character"}
    ]
    if not runner_ids and not articulated_ids:
        return physics_trajectory
    planned_by_frame = {int(frame.get("frame") or 0): frame for frame in planned_trajectory}
    observed_by_frame = {
        int(frame.get("frame") or 0): frame
        for frame in observed_trajectory or []
        if isinstance(frame, dict)
    }
    merged = []
    for frame in physics_trajectory:
        ...
        observed_objects = (observed_by_frame.get(frame_index, {}).get("objects") or {})
        for articulated_id in articulated_ids:
            observed = observed_objects.get(articulated_id)
            if isinstance(observed, dict):
                objects[articulated_id] = observed
...
def rebind_runtime_actors_to_simulation_world(
    ...
    for world in worlds:
        ...
        for actor in world_actors:
            try:
                ...
                pass
        stage_collision_audit = status.setdefault("stage_helper_collision_audit", {})
        for actor_label, actor in label_map.items():
            if actor_label.startswith("native_phenomena_demo_stage_"):
                stage_collision_audit[actor_label] = disable_runtime_stage_helper_collision(actor)
    ...
    initialized_rigs = set(status.get("articulated_control_rig_initialized_ids") or [])
    for obj in (runtime_scene.get("dynamic_objects") or []):
        actor_id = str(obj.get("id") or "")
        if actor_id not in rebound or actor_id in initialized_rigs:
            continue
        if str(obj.get("behavior") or "") != "articulated_character":
            continue
        contract = (obj.get("params") or {}).get("articulated_body") or {}
        if not articulated_control_layers_declared(contract):
            continue
        actor = actors.get(actor_id)
        skeletal = actor_skeletal_component_with_socket(actor, "root") if actor else None
        animation_source = articulated_animation_source_component(actor) if actor else None
        rig = actor.get_component_by_class(unreal.ControlRigComponent) if actor else None
        if not skeletal or not animation_source or not rig:
            status.setdefault("errors", []).append(f"articulated_control_rig_rebind:{actor_id}:components_missing")
            continue
        skeletal_asset_path = str(contract.get("asset_path") or "")
        if not skeletal_asset_path:
            status.setdefault("errors", []).append(f"articulated_control_rig_rebind:{actor_id}:asset_missing")
            continue
        initialize_articulated_control_rig_component(
            rig,
            skeletal,
            animation_source,
            load_asset(skeletal_asset_path),
        )
        initialized_rigs.add(actor_id)
    status["articulated_control_rig_initialized_ids"] = sorted(initialized_rigs)
    ...
    for view_id, frames in sorted(data_pass_frames.items()):
        ...
        if depth_frames:
            depth_frame_indices = [
                ...
                for frame in frames
                if (frame.get("depth") or {}).get("status") == "available"
            ]
            depth_views.append(
                {
                    "view_id": view_id,
                    ...
                    "frame_indices": depth_frame_indices,
        ...
        if segmentation_frames:
            segmentation_frame_indices = [
                int(frame.get("frame") or 0)
                for frame in frames
                if (frame.get("segmentation") or {}).get("status") == "available"
            ]
            segmentation_views.append(
                {
                    ...
                    "frame_indices": segmentation_frame_indices,
                    "source_type": "ue_instance_material_mask",
...
def evaluate_articulated_frame_zero(
    actors: dict,
    runtime_scene: dict | None,
    trajectory: list[dict],
    status: dict,
    ...
    if not trajectory:
        return
    frame = trajectory[0]
    if int(frame.get("frame") or 0) != 0 or abs(float(frame.get("time") or 0.0)) > 1e-9:
        raise RuntimeError("articulated frame-zero initialization requires a t=0 trajectory sample")
    object_ids = articulated_runtime_ids(runtime_scene)
    if not object_ids:
        return
    scene_origin = unreal.Vector(*actors.get("scene_origin", [0.0, 0.0, 0.0]))
    apply_trajectory_frame(actors, object_ids, frame, scene_origin, runtime_scene)
    apply_runtime_animation_segments(actors, runtime_scene, frame, status)
    objects = {
        str(obj.get("id") or ""): obj
        for obj in (runtime_scene or {}).get("dynamic_objects") or []
    }
    calibrate_articulated_grounding(actors, runtime_scene)
    for object_id in object_ids:
        obj = objects.get(object_id) or {}
        if str(obj.get("behavior") or "") != "articulated_character":
            continue
        actor = actors.get(object_id)
        component = actor_skeletal_component_with_socket(actor, "root") if actor else None
        if not component:
            raise RuntimeError(f"articulated frame-zero skeletal component is missing: {object_id}")
        apply_articulated_control_layers(actors, obj, 0.0)
    apply_articulated_attachments(actors, runtime_scene, 0.0)
    sync_runtime_visuals(actors)
    actors["articulated_frame_zero_evaluation"] = {
        "status": "ready",
        "sample_phase": "post_tick",
        "time_s": 0.0,
...
def restore_articulated_frame_zero_roots(actors: dict, runtime_scene: dict | None) -> None:
    initial_transforms = actors.get("runtime_initial_transforms") or {}
    for obj in (runtime_scene or {}).get("dynamic_objects") or []:
        if str(obj.get("behavior") or "") != "articulated_character":
            continue
        object_id = str(obj.get("id") or "")
        actor = actors.get(object_id)
        initial = initial_transforms.get(object_id)
        if not actor or not isinstance(initial, dict):
            raise RuntimeError(f"articulated frame-zero root transform is missing: {object_id}")
        actor.set_actor_location(
            unreal.Vector(*[float(value) for value in initial["position_cm"][:3]]),
            False,
            True,
        )
        actor.set_actor_rotation(runtime_rotator(initial["rotation_degrees"]), False)
...
def start_highres_viewport_capture(
    ...
    state = {
        ...
        "character_tick_pending": None,
        "character_tick_target_time": None,
        "character_advanced_frame": None,
        "articulated_frame_zero_ready": not bool(articulated_runtime_ids(runtime_scene)),
        "camera_tracks": {
            str(output.get("view_id") or output.get("id")): []
            for output in view_outputs
        },
    ...
    summary = {
        ...
        "capture_component_audit": actors.get("capture_component_audit", {}),
    ...
    def start_visible_physics_input() -> None:
        ...
        scripted_ids = list(dict.fromkeys([*runner_ids, *articulated_runtime_ids(runtime_scene)]))
        if scripted_ids:
            apply_trajectory_frame(actors, scripted_ids, first_frame, scene_origin, runtime_scene)
    ...
    def finish():
        ...
        if physics_enabled:
            ...
            if cpp_trajectory:
                state["actual_trajectory"] = merge_scripted_runner_trajectory(
                    cpp_trajectory,
                    trajectory,
                    runtime_scene,
                    observed_trajectory=state["actual_trajectory"],
                )
    ...
    def set_highres_camera_view(location, target, fov: float) -> None:
        ...
        for active_camera in dict.fromkeys((camera, capture_camera)):
            ...
            active_camera.set_actor_rotation(rotation, False)
            active_camera.camera_component.set_editor_property("field_of_view", float(fov))
        summary["capture_component_audit"] = capture_component_audit(
            capture_camera.camera_component, actors, runtime_scene
        )
    ...
    def request_frame(frame_index: int):
        ...
        if (
            ...
            and physics_enabled
            and (
                actors.get("gasp_locomotion")
                or any(
                    obj.get("behavior") == "articulated_character"
                    for obj in (runtime_scene.get("dynamic_objects") or [])
                )
            )
            and state.get("character_advanced_frame") != frame_index
        ):
            character_ids = [
                obj.get("id")
                for obj in (runtime_scene.get("dynamic_objects") or [])
            ...
            apply_trajectory_frame(actors, character_ids, frame, scene_origin, runtime_scene)
            ...
            state["character_tick_pending"] = frame_index
            state["character_tick_target_time"] = now + fixed_physics_step_s(frame_index, FPS)
            return
        if first_view_for_frame and physics_enabled and analytic_contact_solver_enabled(actors, runtime_scene):
            ...
            runner_ids = [obj.get("id") for obj in (runtime_scene.get("dynamic_objects") or []) if obj.get("behavior") == "third_person_runner"] if runtime_scene else []
            scripted_ids = list(dict.fromkeys([*runner_ids, *articulated_runtime_ids(runtime_scene)]))
            if state.get("character_advanced_frame") == frame_index:
                generic_character_ids = {
                    str(obj.get("id") or "")
                    for obj in (runtime_scene.get("dynamic_objects") or [])
                    if obj.get("behavior") == "articulated_character"
                }
                scripted_ids = [object_id for object_id in scripted_ids if object_id not in generic_character_ids]
            if scripted_ids:
                apply_trajectory_frame(actors, scripted_ids, frame, scene_origin, runtime_scene)
            apply_runtime_animation_segments(actors, runtime_scene, frame, physics_status)
            ...
            apply_runtime_animation_segments(actors, runtime_scene, frame, physics_status)
            if state.get("character_advanced_frame") == frame_index:
                for articulated_obj in (runtime_scene.get("dynamic_objects") or []):
                    if str(articulated_obj.get("behavior") or "") == "articulated_character":
                        apply_articulated_control_layers(
                            actors,
                            articulated_obj,
                            float(frame.get("time") or 0.0),
                        )
            ...
            actual_frame, new_events = record_physics_transform_frame(
                ...
                runtime_scene,
                frame_index,
                frame_time_s(frame, frame_index),
                scene_origin,
            ...
            if scripted_ids and isinstance(actual_frame.get("objects"), dict):
                for scripted_id in scripted_ids:
                    scripted_obj = next(
                        (obj for obj in (runtime_scene.get("dynamic_objects") or []) if str(obj.get("id") or "") == str(scripted_id)),
                        {},
                    )
                    if str((scripted_obj.get("params") or {}).get("humanoid_adapter") or "").strip().lower() in {"gasp", "ddv"}:
                        continue
                    contract = (scripted_obj.get("params") or {}).get("articulated_body") or {}
                    if contract:
                        continue
                    if contract.get("mode") == "ragdoll" and float(frame.get("time") or 0.0) >= float(contract.get("ragdoll_start_time_s") or 0.0):
                        continue
                    scripted = (frame.get("objects") or {}).get(scripted_id)
                    if isinstance(scripted, dict):
                        actual_frame["objects"][scripted_id] = {**scripted, "source": scripted.get("source") or "scripted_runtime_preview"}
            state["actual_trajectory"].append(actual_frame)
        ...
        set_highres_camera_view(frame_view["location"], frame_view["target"], frame_view["fov"])
        capture_data_frame = render_data_passes and should_capture_data_frame(frame_index, len(trajectory))
        if capture_data_frame:
        ...
        try:
            ...
            if physics_enabled and not state["physics_ready"]:
                ...
                if not state["physics_ready"] and now - state["physics_wait_started"] > 12.0:
                    ...
                    finish()
                return
            if not state["articulated_frame_zero_ready"]:
                evaluate_articulated_frame_zero(actors, runtime_scene, trajectory, physics_status)
                state["articulated_frame_zero_ready"] = True
                if articulated_frame_zero_needs_post_tick(runtime_scene):
                    actors.setdefault("articulated_frame_zero_evaluation", {}).update({
                        "status": "pending_post_tick",
                        "sample_phase": "pre_tick",
                    })
                summary["articulated_frame_zero_evaluation"] = actors.get("articulated_frame_zero_evaluation")
                summary["articulated_grounding"] = actors.get("articulated_grounding")
                state["runtime_binding_snapshot_ready"] = update_runtime_binding_snapshot()
                state["runtime_binding_wait_started"] = time.perf_counter()
                write_summary(summary)
                return
            if not state["runtime_binding_snapshot_ready"]:
                if update_runtime_binding_snapshot():
                    ...
                    return
            if state.get("character_tick_pending") is not None:
                pending_frame = int(state["character_tick_pending"])
                world = actors.get("physics_game_world")
                target_time = float(state.get("character_tick_target_time") or 0.0)
                ...
                if current_time + 1e-4 < target_time:
                    return
                for articulated_obj in (runtime_scene or {}).get("dynamic_objects") or []:
                    if str(articulated_obj.get("behavior") or "") != "articulated_character":
                        continue
                    object_id = str(articulated_obj.get("id") or "")
                    actor = actors.get(object_id)
                    movement = actor.get_component_by_class(unreal.CharacterMovementComponent) if actor else None
                    if movement:
                        velocity = movement.velocity
                        actors.setdefault("articulated_character_movement_state", {}).setdefault(
                            object_id, {}
                        )["velocity_cm_s"] = [float(velocity.x), float(velocity.y), float(velocity.z)]
                set_simulation_world_paused(actors, physics_status, True)
                state["character_tick_pending"] = None
                state["character_tick_target_time"] = None
                state["character_advanced_frame"] = pending_frame
                ...
                pending_sample = trajectory[pending_frame]
                pending_time = float(pending_sample.get("time") or 0.0)
                if pending_frame == 0:
                    restore_articulated_frame_zero_roots(actors, runtime_scene)
                    apply_trajectory_frame(
                        actors,
                        articulated_runtime_ids(runtime_scene),
                        pending_sample,
                        scene_origin,
                        runtime_scene,
                    )
                    apply_runtime_animation_segments(
                        actors,
                        runtime_scene,
                        pending_sample,
                        physics_status,
                ...
                for articulated_obj in (runtime_scene or {}).get("dynamic_objects") or []:
                    if str(articulated_obj.get("behavior") or "") == "articulated_character":
                        apply_articulated_control_layers(actors, articulated_obj, pending_time)
                apply_articulated_attachments(actors, runtime_scene, pending_time)
                if pending_frame == 0:
                    actors.setdefault("articulated_frame_zero_evaluation", {}).update({
                        "status": "ready",
                        "sample_phase": "post_tick",
                    })
                    summary["articulated_frame_zero_evaluation"] = actors.get(
                        "articulated_frame_zero_evaluation"
                    )
                physics_status["character_world_tick_count"] = int(physics_status.get("character_world_tick_count") or 0) + 1
                return
...
def main():
    ...
    for frame in trajectory:
        ...
        for view in view_outputs:
            ...
            if RENDER_DATA_PASSES and should_capture_data_frame(frame_index, len(trajectory)):
                data_pass_frames.setdefault(view_id, []).append(
                    export_depth_and_segmentation_frame(actors, runtime_scene, view_id, frame_index, data_pass_dirs)
                )
        if frame_index == 0 and RENDER_DATA_PASSES and should_capture_data_frame(frame_index, len(trajectory)):
    ...
    if physics_enabled:
        cpp_trajectory, cpp_contact_events = stop_cpp_runtime_driver(actors, physics_status, runtime_scene, scene_origin)
        if cpp_trajectory:
            actual_trajectory = merge_scripted_runner_trajectory(
                cpp_trajectory,
                trajectory,
                runtime_scene,
                observed_trajectory=actual_trajectory,
            )
    ...
    summary = {
        ...
        "runtime_ground_offsets": actors.get("runtime_ground_offsets", {}),
        "runtime_initial_transforms": actors.get("runtime_initial_transforms", {}),
        "articulated_frame_zero_evaluation": actors.get("articulated_frame_zero_evaluation"),
        "articulated_grounding": actors.get("articulated_grounding"),
        "runtime_binding_snapshot": actors.get("runtime_binding_snapshot", {}),
        "chaos_runtime": actors.get("chaos_runtime", {}),
        "removed_map_actors": actors.get("removed_map_actors", []),
        "background_stage_actors": actors.get("background_stage_actors", []),
